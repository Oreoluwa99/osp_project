 ...
 * modified by Alan Denton 09/17/08, 09/17/22
 * @version 1.0  05/16/05
 */
public class Projectile implements Drawable, ODE {
  static final double g = 9.8; // acceleration due to gravity
  double[] state = new double[5]; // {x,vx,y,vy,t}
  static double c2; // quadratic drag coefficient
  static double x0, y0, vx0, vy0; // initial position and velocity 
  double x1, y1, vx1, vy1; // position and velocity with no drag
  double t; // time
  int pixRadius = 6; // pixel radius for drawing of projectile
  EulerRichardson odeSolver = new EulerRichardson(this); // ODE solver algorithm
  ...
  public void getRate(double[] state, double[] rate) {
    ...
    double v = Math.sqrt(rate[1]*rate[1]+rate[3]*rate[3]); // speed
    rate[1] = -c2*v*rate[0]; // rate of change of vx (quadratic drag)
    rate[2] = state[3]; // rate of change of y
    rate[3] = -g - c2*v*rate[2]; // rate of change of vy (quadratic drag)
    rate[4] = 1; // dt/dt = 1

    t = state[4]; // time
    x1 = x0+vx0*t; // position and velocity with no drag
