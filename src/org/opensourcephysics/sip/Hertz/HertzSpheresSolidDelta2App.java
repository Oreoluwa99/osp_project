 ...
 */
 ...
 public class HertzSpheresSolidDelta2App extends AbstractSimulation {
...
//	HertzSpheresSolidPhaseTrialCode particles = new HertzSpheresSolidPhaseTrialCode();
	...
	public void doStep() { // logical step in the HertzSpheres class	    	    
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("Initial Energy U_0 = " + particles.initialEnergy);
				...
				//	totalPairEnergy = -(ePairSum*particles.dlambda);
				//	totalPairEnergyPerVolume = (totalPairEnergy*particles.N)/totalVol;
					springEnergyAccumulator1 = -(springEnergyAccumulator*particles.dlambda*particles.N);
				...
				//	initalEnergy = (particles.initialEnergy*particles.N)/(double)totalVol;
					...
					double firstPart = 1.5*(1-1/(double)particles.N)*Math.log((particles.springConstant*Math.pow(totalVol, 2/(double)3))/Math.PI);
					double secondPart = (1+2/particles.N)*Math.log(particles.N);
					double thirdPart = Math.log(2*Math.PI)/2*particles.N;

					part1 = particles.initialEnergy/(double)particles.N;
					...
					double idealFreeEnergyPerVolume = ((0.75/Math.PI)*dryVolFrac*(Math.log(dryVolFrac))-1.0);
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidDelta2App());
	}
}
