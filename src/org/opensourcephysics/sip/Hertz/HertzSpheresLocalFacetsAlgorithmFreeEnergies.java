/*
 * The major reason for this code is to account for the local faceting of the microgels in the step() method.
 * In this code, the local faceting of the microgels are all set up but I started from there 
 * and added the free energy calculations
 */
...
public class HertzSpheresLocalFacetsAlgorithmFreeEnergies {
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (i = 0; i < N; i++) { // one microgel
         ...
         for (j = 0; j < N; j++) {
            ...
            //System.out.println("Math.log(1-1/(a[j]*a[j]*a[j])): "+(Math.log(1-1/(a[j]*a[j]*a[j]))));
            elasticF = 1.5*newNMon*xLinkFrac*(a[j]*a[j]-Math.log(a[j])-1);
            floryFRJBeforeMove[j] = mixF+elasticF;

            //System.out.println("floryFRJBeforeMove: "+floryFRJBeforeMove[j]);
            //System.out.println("floryRehnerBeforeMove: " + floryFRJBeforeMove[j] + " for microgel: " + j);
            }
         }
         
         // flory-Rehner free energy for i
         newNMon = nMon*(volOfMicrogel[i]-capVolSumBeforeMove)/volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
         ...
         //System.out.println("floryFRIBeforeMove: "+floryFRIBeforeMove);
         ...
         //System.out.println("floryFRIAfterMove: "+floryFRIAfterMove);

         double dFloryForI = floryFRIAfterMove-floryFRIBeforeMove; // change in energy for microgel i
         //System.out.println("dFloryForI: "+dFlory);

         // change in energy for microgel j
         double dFloryForJ = 0;
         for(j = 0; j < N; j++){
            if(j != i){
               dFloryForJ += (floryFRJAfterMove[j]-floryFRJBeforeMove[j]);
               //System.out.println("dFloryForJ: "+dFloryForJ);
            }
         }
         //System.out.println("dFloryForJ: "+dFloryForJ);

         dFlory =dFloryForI+dFloryForJ; //total change in FR for the system
         //System.out.println("totaldFlory: "+dFlory);
   ...
   public double meanRadius() { // mean radius [units of dry radius]
    ...
    public double reservoirSwellingRatio(double nMon, double nChains, double chi){
      HertzSpheresLocalFacetsAlgorithmFreeEnergies.FloryRehnerPressure f = new FloryRehnerPressure(nMon, nChains, chi);
      double xleft = 1.1;
      double xright = 15.;
