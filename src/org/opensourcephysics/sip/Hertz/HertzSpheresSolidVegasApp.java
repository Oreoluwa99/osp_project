 ...
 */
 ...
 public class HertzSpheresSolidVegasApp extends AbstractSimulation {
    public enum WriteModes{WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
//	HertzSpheresSolidVegas particles = new HertzSpheresSolidVegas();
	HertzSpheresSolidVegasEdited particles = new HertzSpheresSolidVegasEdited();
//	HertzSpheresSolidPhaseEdited particles = new HertzSpheresSolidPhaseEdited();
	...
	double a0overNPerVolume;
	double delta1, delta2, delta1PerVolume, delta2PerVolume;
	double a0, a0overN;
	double idealEinsteinFreeEnergyPerVolume;
	double boltzmannFactorAccumulatorPerSteps;
	double helmhotzFreeEnergyPerVolume, helmhotzFreeEnergy;
	double springEnergySumAccumulator=0;
	double uLattice, boltzmannFactorPerN, einsteinFreeEnergy, idealContribution1, idealContribution2;
	double totalFreeEnergy, totalFreeEnergyPerVolume;
	...
	List<Double> excessHelmhotzFreeEnergyPerVolumes = new ArrayList<>();
	List<Double> initialEnergyList = new ArrayList<>();
	List<Double> ULattices = new ArrayList<>();
	List<Double> boltzmannFactorPerNList = new ArrayList<>();
	List<Double> a0overNPerVolumeList = new ArrayList<>();
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("Initial Energy U_0 = " + particles.initialEnergy);
				...
				if (lambda==0){
					...
					uLattice=particles.initialEnergy/particles.N;
					//	boltzmannFactorAccumulatorPerSteps = particles.boltzmannFactorAccumulator/particles.numberOfConfigurations;
					...
					delta1 = uLattice-particles.boltzmannFactor();
	...
	public void writeData(){
		...
		try {
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, totalSumOfEnergies, delta1, delta2, floryRehnerFreeEnergy/V, ULattice/N, a0overNPerVolume");
			bw1.newLine();	

			for (int i = 0; i < dryVolFracs.size(); i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + totalSumOfEnergiesList.get(i)+ ", " + delta1List.get(i) + ", " + delta2List.get(i) + ", " + meanFreeEnergyPerVolumes.get(i) + ", " + ULattices.get(i) + ", " + a0overNPerVolumeList.get(i));
				bw1.newLine();
			}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidVegasApp());
	}
}
