...
public class HertzSpheresLiquidLambdaEditedCopyApp extends AbstractSimulation {
    public enum WriteModes{WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
    HertzSpheresLiquidLambdaEdited particles = new HertzSpheresLiquidLambdaEdited();
	...
	double totalVol;
	double pairPressure, pairPressureContribution;
	double freeEnergyPerVolume, pairEnergyTotalPerVolume, idealFreeEnergyPerVolume;
	double initialFreeEnergyTotalPerVolume;
	double totalCalculatedPressure;
	double floryPressureContribution;
	boolean isFirstIteration;
	double initialpairEnergyTotalPerVolume;
	double stirlingApproximation;
	double idealPlusPairFreeEnergy;
	double initialIdealPlusPairEnergy;
	double initialVolume;
	double density;
	double latestpairEnergyTotalPerVolume, latestFreeEnergyTotalPerVolume, latestIdealPlusPairEnergy, latestVolume;
	double deltaFpairDividedByDeltaPhi, deltaFfloryRehnerbyDeltaPhi, idealPlusPairEnergyChange, volumeChange;
	double freeEnergyPressure, idealPlusPairEnergyChangePerVolumeChange;
	double initialFE, latestFE, chemicalPotential, dFRFE_dPhi, dFPair_dPhi;
	double floryPressure, floryPContribution, pairPContribution;
	double uPairPerVol;
	...
	List<Double> idealPlusPairEnergyChangePerVolumeChanges = new ArrayList<>();
	List<Double> freeEnergyPerVolumes = new ArrayList<>();
	List<Double> idealFreeEnergyPerVolumes = new ArrayList<>();
	List<Double> meanPressures = new ArrayList<>();
	List<Double> idealAndPairEnergies = new ArrayList();
	List<Double> pairPressures = new ArrayList<>();
	List<Double> totalPressures = new ArrayList<>();
	List<Double> floryPressures = new ArrayList<>();
	List<Double> stirlingApproximations = new ArrayList<>();
	List<Double> reservoirVolFracs = new ArrayList<>();
	List<Double> virialPressures = new ArrayList<>();
	...
	*/
	public void initialize() {
		...
		*/
		...
		/* 
		if (isFirstIteration) {
			initialpairEnergyTotalPerVolume = pairEnergyTotalPerVolumes.get(0); // first pair energy in the list
			initialFreeEnergyTotalPerVolume = freeEnergyPerVolumes.get(0); // first free energy in the list
			initialIdealPlusPairEnergy = idealAndPairEnergies.get(0); // first ideal and pair free energy
			initialVolume = totalVolList.get(0);
			...
			if (dryVolFrac < dryVolFracMax){
				...
				//	control.println("<Epair>/lambda = " + decimalFormat.format(particles.meanPairEnergy()/particles.lambda));
					...
					if (lambda == 1){
						...
						pairEnergyTotal = EpairSum * particles.dlambda; // calculate the totalPairEnergy
						pairEnergyTotalPerVolume = (pairEnergyTotal * particles.N)/totalVol; 
						idealFreeEnergyPerVolume = (0.75/Math.PI)*density*(Math.log(density)-1.0);
						freeEnergyPerVolume = (particles.meanFreeEnergy() * particles.N)/totalVol; // the flory-Rehner free energy per volume
						stirlingApproximation = (0.75/Math.PI)*dryVolFrac*Math.log(2*Math.PI*particles.N)/(2*particles.N); // approximation to the free energy
						idealPlusPairFreeEnergy = idealFreeEnergyPerVolume + stirlingApproximation + pairEnergyTotalPerVolume * (totalVol / particles.N);
						totalSum = (pairEnergyTotalPerVolume + freeEnergyPerVolume + idealFreeEnergyPerVolume+stirlingApproximation); // the addittion of all the free energies
						uPairPerVol = particles.meanPairEnergy()*(particles.N/particles.totalVol);
						

						/* Update the array with the latest values of dryVolFrac and 
						totalSum, free energy and total pair energy, among others */
						dryVolFracs.add(dryVolFrac);
						totalSums.add(totalSum);
						idealFreeEnergyList.add(idealFreeEnergyPerVolume);

					//	freeEnergyTotals.add(particles.meanFreeEnergy());
						floryFperVolList.add(freeEnergyPerVolume);
						pairEperVolList.add(pairEnergyTotalPerVolume);
						uPairPerVolList.add(uPairPerVol);

						/*set these values to the next values in the list*/

						// calculate the difference between the latest values and the initial values with respect to the dry volume fraction
						deltaFpairDividedByDeltaPhi = (pairEnergyTotalPerVolume-initialpairEnergyTotalPerVolume)/particles.dphi;
						deltaFfloryRehnerbyDeltaPhi = (freeEnergyPerVolume-initialFreeEnergyTotalPerVolume)/particles.dphi;
					...
					if (lambda < 1){
						/* Accumulate all the pair energies */
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergyLambdaIntegration.txt");
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresLiquidLambdaEditedCopyApp());
	}
}
