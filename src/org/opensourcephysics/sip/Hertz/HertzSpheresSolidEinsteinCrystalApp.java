 ...
 */
 ...
 public class HertzSpheresSolidEinsteinCrystalApp extends AbstractSimulation {
...
//	HertzSpheresSolidPhaseTrialCode particles = new HertzSpheresSolidPhaseTrialCode();
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("Initial Energy U_0 = " + particles.initialEnergy);
				...
				//	totalVolumes.add(particles.totalVol);
					...
					particles.lambda = lambda;
					springEnergySumAccumulator = 0; // reset springEnergyAccumulator to 0
					this.initialize();
					return;
				}
	...
	public void writeData(){
		...
		try {
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, totalSumOfEnergies, delta1, delta2, floryRehnerFreeEnergy/V, ULattice/N, boltzmannFactor/N, a0overNPerVolume");
			bw1.newLine();	

			for (int i = 0; i < dryVolFracs.size(); i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + totalSumOfEnergiesList.get(i)+ ", " + delta1List.get(i) + ", " + delta2List.get(i) + ", " + meanFreeEnergyPerVolumes.get(i) + ", " + ULattices.get(i) + ", " + boltzmannFactorPerNList.get(i) + ", " + a0overNPerVolumeList.get(i));
				bw1.newLine();
			}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidEinsteinCrystalApp());
	}
}
