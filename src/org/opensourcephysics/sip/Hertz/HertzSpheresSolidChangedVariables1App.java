 ...
 */
 ...
 public class HertzSpheresSolidChangedVariables1App extends AbstractSimulation {
...
//	HertzSpheresSolidEinsteinCrystal particles = new HertzSpheresSolidEinsteinCrystal();
	...
	double a0, a0overN, a0overNPerVolume;
	double x, floryRehnerfreeEnergyPerVolume;
	double boltzmannFactorAccumulatorPerSteps, boltzmannFactorPerN;
	double delta1, delta1PerVolume;
	double uLattice;
	double totalFreeEnergy, totalFreeEnergyPerVolume, totalSumOfEnergiesPerVolume;
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("The coupling constant = "+lambda);
				...
				if (x<upperLimit){
					...
					if (x>upperLimit){
						/* calculate delta 2 */
						delta2 = -(delta2Accumulator*xIncrement)/(double)particles.numberOfConfigurations/particles.N;
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergySolidPhaseEdited.txt");
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, totalFreeEnergies, delta1, delta2, a0, meanFreeEnergy");
			bw1.newLine();	

			for (int i = 0; i < dryVolFracs.size(); i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + totalSumOfEnergiesList.get(i) + ", " + delta1List.get(i) + ", " + delta2PerVolumeList.get(i) + ", " + a0overNPerVolumeList.get(i) + ", " + meanFreeEnergyPerVolumes.get(i));
				bw1.newLine();
			} 
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidChangedVariables1App());
	}
}
