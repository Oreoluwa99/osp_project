 ...
 * rather than “crushing” the polymer in the cap regions. The step() method is where the change took place. 
 */
 ...
 public class HertzSpheresNonLocalFacet_AlansEdits {
    ...
    */
    ...
    public void step() { // performs a trial move of every particle
       ...
       for (i = 0; i < N; i++) {
         ...
         //System.out.println("\nBefore Move - Microgel " + i);
         //System.out.println("x: " + x[i] + ", y: " + y[i] + ", z: " + z[i] + ", a: " + a[i]);
...
// resume 
          ...
          //de = pairEnergySum - energy[i] + dFlory; # the original dFlory
...
//
          ...
          if (capVolumesBeforeMove[i] < -1e-10) { System.out.println("STOP: capVolumesBeforeMove[i] < 0 !!!");} // Alan: check for negative cap volumes
             //capVolSumBeforeMoveArray[i] = capVolSumAfterMoveI; // Alan commented out
 
             // Update old free energy for microgel i before the next trial move
/* Alan: moved up above, so that j!=i
             for (j = 0; j < N; j++) { 
...
*/
    ...
    public double reservoirSwellingRatio(double nMon, double nChains, double chi){
       ...
       HertzSpheresNonLocalFacet_AlansEdits.FloryRehnerPressure f = new FloryRehnerPressure(nMon, nChains, chi);
       double xleft = 1.1;
       double xright = 15.;
