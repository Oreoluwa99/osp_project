...
public class HertzSpheresCopyWithoutLambda {
    ...
    public double currentX[], currentY[], currentZ[];//stores the initial random positions
    ...
    public double dryVolFrac, volFrac, reservoirVolFrac, dryVolFracStart; 
    ...
    public double B1, derivPart1, HertzEnergy1, pairEnergySum1, totalPairEnergy1, pairEnergyAccumulator1;
    public double totalEnergy1, energyAccumulator1, fOverR1;
    public double fx1, fy1, fz1, virialSum1, totalVirial1, virialAccumulator1, initialEnergy;
    public double displacement, trialDisplacementDistance;
   ...
   public void initialize(String configuration) {
      ...
      CM_initialX = new double[N];
      CM_initialY = new double[N];
      CM_initialZ = new double[N];
      ...
      */
      ...
      //Array to store initial random positions
      currentX = new double[N]; 
      currentY = new double[N];
      currentZ = new double[N];
   ...
   public void setFCCpositions() {
            ...
            for (iz = 0; iz < 2*nx; iz++) { // loop through particles in a layer
               if (i < N) { // check for remaining particles
                  if ((ix+iy+iz)%2 == 0) { // check for remaining particles
                     ...
                     /*
                     // Accumulate the positions for calculating center of mass
                     sumX += x[i];
                     sumY += y[i];
                     sumZ += z[i];
                     System.out.println("sumX " + sumX[i]);
                     */
                  }
               }
            }
      ...
      CM_initialX[i] = sumX[i]/N;
      CM_initialY[i] = sumY[i]/N;
      CM_initialZ[i] = sumZ[i]/N;
      */
    //  System.out.println("CM_initialX = " + CM_initialX + "CM_initialY = " + CM_initialY + "CM_initialZ = " + CM_initialZ);
      calculateTotalEnergy(lambda); // initial energy 
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
         ...
         //chanage in displacment per particle trial move: delta r/N
      ...
      calculateTotalEnergy(lambda); // new total 0energy
	  
   }

   public void calculateTotalEnergy(double lambda) {
    //  System.out.println("CalculateTotalEnergy");
      totalEnergy = 0;
            ...
            if(j != i) { // consider interactions with other particles
               ...
               //   pairEnergy[i][j] = HertzEnergy; // pair energy of particles i and j
		         ...
		         //   fOverR = (2.5/sigma)*derivPart/r;
                  fOverR1 = (2.5/sigma)*derivPart1/r;
               /*
               ...
               */
                  ...
                  fy1 = fOverR1*yij;
                  fz1 = fOverR1*zij;
                  virialSum1 += xij*fx1 + yij*fy1 + zij*fz1;
	            }
            }
         }
...
//    pairEnergyAccumulator += totalPairEnergy;
      ...
      boltzmannFactor = Math.exp((initialEnergy/(double)(steps))/N-pairEnergyAccumulator1/(double)(steps)/N);
      pairPotentialDifferenceAccumulator += boltzmannFactor;
      System.out.println("difference = " + (initialEnergy-pairEnergyAccumulator1));
      System.out.println("boltzmannFactor = " + boltzmannFactor);
      System.out.println("initialEnergy = " + initialEnergy);
      System.out.println("pairEnergyAccumulator = " + pairEnergyAccumulator1);
...
//    virialAccumulator += totalVirial; //for the lambda system
      virialAccumulator1 += totalVirial1;
...
*/ 
   ...
   public double meanPairEnergy() {
      return pairEnergyAccumulator1/N/(double)(steps); // quantity <E_pair>/N
   }
