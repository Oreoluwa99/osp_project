/*
 * This code is an extension of the HertzSpheresSolidPhaseGaussLegendreApp, but branced solely for 
 * the purpose of accounting for the free energies of the microgels in the faceting algorithm case
 * 
 */
...
public class HertzSpheresSolidPhaseGaussLegendreFacetingApp extends AbstractSimulation {
	...
	public void initialize() {
		...
		//xLinkFrac = control.getDouble("x-link fraction");
		...
		if (particles.steps == particles.stop) {
			...
			if (dryVolFrac < dryVolFracMax) {
				if (lambda == 1) { // the real or interacting solid
					...
					deltaFAccumulator += (0.5*gaussWeight)*(Math.log(particles.springConstant+Math.exp(3.5))-3.5)*(Math.exp(variableChanged))/particles.springConstant*(-particles.meanSpringEnergy()); //<U_pair - U_spring>
	...
	public void reset() {
		...
		control.setValue("DryVolFrac Max", 0.015);
	...
	public void writeData() {
		...
		try {
			...
			File outputFile = new File("data/Comprehensive_Report_Data/Non_Local_Facet/Free_Energies_xlink_0.001"+particles.fileExtension+".txt");
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidPhaseGaussLegendreFacetingApp());
	}
}
