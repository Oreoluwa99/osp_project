...
public class HertzSpheresSecondInterpenetrationForFreeEnergies {
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
         ...
         //de = ((pairEnergySum+mixF_Interpenetration)-energy[i])+(newFloryFR-oldFloryFR); // change in total energy due to trial move
         
         de = lambda*(mixF_Interpenetration-energy[i])+(1-lambda)*(pairPotentialCorrection)+(newFloryFR-oldFloryFR);
