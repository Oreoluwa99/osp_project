/*
 * This code is all set up for the faceting algorithm and works just fine now!
 * 
 */
...
public class HertzSpheresInterpenetrationAlgorithmApp extends AbstractSimulation {
	
	public enum WriteModes {WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
	//HertzSpheresInterpenetrationAlgorithm particles = new HertzSpheresInterpenetrationAlgorithm();
	
	// the real code to be used
	...
	//HertzSpheresNonLocalFacet2 particles = new HertzSpheresNonLocalFacet2();
	//HertzSpheresFacetingAlgorithm particles = new HertzSpheresFacetingAlgorithm();


	HertzSpheresSecondInterpenetrationForFreeEnergies particles = new HertzSpheresSecondInterpenetrationForFreeEnergies();
	...
	List<Double> xLinkFracList = new ArrayList<>();
	...
	public void initialize() {
		...
		xLinkFracStart = control.getDouble("x-link fraction");
		...
		if (incrementXLinkFrac){
			xLinkFrac = xLinkFracStart;
			particles.xLinkFrac = xLinkFrac;
			incrementXLinkFrac = false;
		}
		...
		if (particles.steps == particles.stop) {
			if (xLinkFrac < xLinkFracMax){
				...
				if (dryVolFrac < dryVolFracMax) {
					...
					if(lindemannParameter<0.15){						
						control.println("phi_0: "+dryVolFrac);
						control.println("x-Link Fraction: "+xLinkFrac);
						control.println("L: "+lindemannParameter);
						control.println("alpha: "+particles.meanRadius());
						control.println("phi: "+(dryVolFrac*Math.pow(particles.meanRadius(), 3)));
						control.println("kT/E: "+(1.0/particles.B));
						...
						volumefractionList.add(dryVolFrac*Math.pow(particles.meanRadius(), 3));
						xLinkFracList.add(xLinkFrac);
						softnessList.add(1.0/particles.B);

						// increment the x-Link fraction
						xLinkFrac += dxLink;
						particles.xLinkFrac = xLinkFrac;
						// reset the dry volume fraction
						dryVolFrac = dryVolFracStart;
						particles.dryVolFrac = dryVolFrac;
	...
	public void reset() {
		...
		control.setValue("x-link fraction max", 0.0001);
		control.setValue("x-Link increment", 0.00000175);
		//control.setValue("x-Link increment", 0.000003);
	...
	public void writeData() {
		...
		try {
			...
			File dir = new File("data/ISMC_DATA");
			if (!dir.exists()) {
				if (dir.mkdirs()) {
			...
			File outputFile = new File(dir, "StabilityInterpenetration" + particles.fileExtension + ".txt");
			...
			bw1.newLine();
			bw1.write("#Max Dry volume fraction: " + dryVolFracMax);
			bw1.newLine();
			...
			//bw1.newLine();
			bw1.write("#phi0,		   phi,       	   <alpha>,      		zeta, 	 	 	  lindemannParameter, 			x-link, 		kT/E");
			bw1.newLine();

			for (int i = 1; i < dryVolFracs.size() - 1; i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + volumefractionList.get(i) + ", " + swellingRatioList.get(i) + ", " + reservoirVolFracList.get(i) + ", " + lindemannParameterList.get(i) + ", " + xLinkFracList.get(i) + ", " + softnessList.get(i));
				bw1.newLine();
			}
			bw1.close();
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresInterpenetrationAlgorithmApp());
	}
}
