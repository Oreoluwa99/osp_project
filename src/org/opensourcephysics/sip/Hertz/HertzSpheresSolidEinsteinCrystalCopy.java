...
public class HertzSpheresSolidEinsteinCrystalCopy {
   ...
   public void setFCCpositions() {
      ...
      calculateTotalEnergy(lambda); // initial energy 
      initialEnergy = totalPairEnergy;// when lambda = 1
   }
   ...
   */
      ...
      for(int i = 0; i < N; ++i) {
         ...
         if (steps > delay){
            if ((steps-delay)%snapshotInterval==0){ // include configurations only after even number of intervals
               ...
               System.out.println("Number of configurations = "+numberOfConfigurations);
   ...
   public double meanboltzmannFactor(){
      return boltzmannFactorAccumulator/N/(double)steps;
   }

   public double meanSpringEnergyAccumulator(){
      return springEnergyAccumulator/N/(double)steps;
   }
