 ...
 * It is all set up for Faceting and DOES NOT YET work just fine
 */
...
public class HertzSpheresNonLocalFacet2_Edited {
    ...
    public double capVolAfterMove[][], capVolBeforeMove[][]; // stores the cap volume for each j microgel before and after a move
   ...
   public void initialize(String configuration) {
      ...
      capVolAfterMove = new double[N][N];
      capVolBeforeMove = new double[N][N];
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
         ...
         for (j = 0; j < N; j++) {
            ...
            if (j != i) {
               ...
               if (r2 < sigma * sigma) {
                  ...
                  capVolBeforeMove[i][j] = capVolForJ;
                  //capVolBeforeMove[j][i] = capVolForJ;
                  capVolBeforeMove[j][i] = capVolForI; // Alan
               }
               else{ //if they are not overlapping
                  capVolBeforeMove[i][j] = 0;
                  capVolBeforeMove[j][i] = 0;
               }
            }
         }
      ...
      for (i = 0; i < N; i++) {
         ...
         dxtrial = tolerance * 2.0 * (random.nextDouble() - 0.5); // Alan: deterministic random number generator with seed
         ...
         for (j = 0; j < N; j++) {
            ...
            if (j != i) {
               ...
               if (r2 < sigma * sigma) {
                  ...
                  capVolForJ = (Math.PI * Math.pow(hj, 2) * (3.0 * a[j] - hj)) / 3.0;
                  capVolForI = (Math.PI * Math.pow(hi, 2) * (3.0 * a[i] - hi)) / 3.0;
                  // capVolJAfterMove[j] = capVolForJ;
                  capVolSumAfterMoveI += capVolForI;
                  //capVolSumAfterMoveJ += capVolForJ;
                  capVolAfterMove[i][j] = capVolForJ;
                  //capVolAfterMove[i][j] = capVolForJ;
                  capVolAfterMove[j][i] = capVolForI; // Alan

                  // Calculate FR free energy for microgel j after the move
                  double newCapVolJ = capVolSumBeforeMoveArray[j] + capVolAfterMove[i][j] - capVolBeforeMove[i][j];
                  capVolSumAfterMoveArray[j] = newCapVolJ;
                  newSWR = a[j] * Math.pow(((volOfMicrogel[j] - capVolSumAfterMoveArray[j]) / volOfMicrogel[j]), 1.0 / 3.0);
                  mixF = nMon * ((newSWR * newSWR * newSWR - 1) * Math.log(1 - 1 / (newSWR * newSWR * newSWR)) + chi * (1 - 1 / (newSWR * newSWR * newSWR)));
                  elasticF = 1.5 * nMon * xLinkFrac * (newSWR * newSWR - Math.log(newSWR) - 1);
                  ...
                  capVolAfterMove[i][j] = 0;
                  capVolAfterMove[j][i] = 0;
         ...
         //capVolSumAfterMoveArray[i] = capVolSumAfterMoveI;
         ...
         if (Math.exp(-de) < random.nextDouble()) { // Metropolis algorithm
            ...
            for (j = 0; j < N; ++j) {
               if (j != i) {
                  ...
                  capVolBeforeMove[i][j] = capVolAfterMove[i][j];
                  //capVolBeforeMove[j][i] = capVolAfterMove[i][j];
                  capVolBeforeMove[j][i] = capVolAfterMove[j][i]; // Alan
               }
            }
            ...
            //floryFRBeforeMove[i] = floryFRAfterMove[i]; // Alan
            for (j = 0; j < N; j++) { // Alan
               floryFRBeforeMove[j] = floryFRAfterMove[j];
               if(j != i){
                  // update cap volume sums
                  capVolSumBeforeMoveArray[j] = capVolSumAfterMoveArray[j];
               }
            }  
         }
      }
   ...
   public double reservoirSwellingRatio(double nMon, double nChains, double chi){
      HertzSpheresNonLocalFacet2_Edited.FloryRehnerPressure f = new FloryRehnerPressure(nMon, nChains, chi);
      double xleft = 1.1;
      double xright = 15.;
