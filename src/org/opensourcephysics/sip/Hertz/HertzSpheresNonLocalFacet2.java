...
public class HertzSpheresNonLocalFacet2 {
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (i = 0; i < N; i++) {
         ...
         if (Math.exp(-de) < random.nextDouble()) { // Metropolis algorithm
            ...
            // Update old free energy for all other microgels j
            for (j = 0; j < N; j++) {
               if (j != i) {
   ...
   public double meanRadius() { // mean radius [units of dry radius]
    ...
    public double reservoirSwellingRatio(double nMon, double nChains, double chi){
      HertzSpheresNonLocalFacet2.FloryRehnerPressure f = new FloryRehnerPressure(nMon, nChains, chi);
        double xleft = 1.1;
        double xright = 15.;
