...
public class HertzSpheresSolidVegas {
   ...
   public void initialize(String configuration) {
      ...
      newPairEnergy1 = new double[N][N]; // for lambda = 1
      steps = 1;
      energyAccumulator = 0;
      ...
      for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
         ...
         for (int j = 0; j < N; j++) {
            if(j != i) { // consider interactions with other particles
               ...
               //   pairEnergySum1+=HertzEnergy;
                //  newEnergy += pairEnergySum1;
                  */
                  ...
                  newPairEnergy[i][j] = HertzEnergy1;
      ...
      //      System.out.println("Move rejected");
       ...
       //      System.out.println("Move accepted");
            ...
            for (int j = 0; j < N; ++j) { // update energies of other particles 
               if(j != i) {
                  energy[j] += newPairEnergy[i][j]-pairEnergy1[i][j];
                  pairEnergy1[i][j] = newPairEnergy[i][j];
                  pairEnergy1[j][i] = newPairEnergy[i][j];
               }
            }
         }
      }
...
*/
   ...
   // System.out.println("initialEnergy-totalPairEnergy1 = " +(initialEnergy-totalPairEnergy1));
      if (lambda == 0){
         ...
         System.out.println("totalPairEnergy1 = "+totalPairEnergy1);
