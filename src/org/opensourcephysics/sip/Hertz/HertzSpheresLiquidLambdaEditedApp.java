...
public class HertzSpheresLiquidLambdaEditedApp extends AbstractSimulation {
	...
	double dFRFE_dPhi;
	double initialFloryRehnerFreeEnergy, latestFloryRehnerFreeEnergy;
	double floryRehnerFreeEnergyChange, floryRehnerFreeEnergyChangePerDryVolFrac, freeEnergyChangePerDryVolFracChange;
	...
	List<Double> chemicalPotentialsList = new ArrayList<>();
	...
	*/
	public void initialize() {
		...
		*/
		if (isFirstIteration) {
			initialFreeEnergy = totalSums.get(0);
			initialFloryRehnerFreeEnergy = freeEnergyPerVolumes.get(0); // the first flory-Rehner free energy in the list
		...
		if (particles.steps == particles.stop){
			...
			if (dryVolFrac < dryVolFracMax){
					...
					if (lambda == 1){
						...
						freeEnergyPerVolume = (particles.meanFreeEnergy() * particles.N)/totalVol; // flory-Rehner free energy
					...
					//	latestFloryRehnerFreeEnergy = freeEnergyPerVolumes.get(freeEnergyPerVolumes.size()-1); // latest floryRehner free energy per volume

						floryRehnerFreeEnergyChange = freeEnergyPerVolume - initialFloryRehnerFreeEnergy; // floryRehner free enrgy change

						dFRFE_dPhi = (floryRehnerFreeEnergyChange/(2*particles.dphi));

					//	set latestpairEnergyTotalPerVolume to the next value in the list of pairEnergyTotalPerVolumes
					// 	double latestFreeEnergy = totalSums.get(totalSums.size()-1);
					// 	calculate the difference between the first two pairEnergyTotalPerVolumes with respect to the dry volume fraction
						double freeEnergyChange = (totalSum - initialFreeEnergy)/particles.dphi;
						
						freeEnergyChangePerDryVolFracChange = ((freeEnergyChange)*dryVolFrac - totalSum);
					//	the addition of the ideal gas pressure and pair energy pressure
						pairPressure = (freeEnergyChangePerDryVolFracChange * totalVol)/particles.N + 1.0;
		
					// 	Update initialpairEnergyTotalPerVolume to latestpairEnergyTotalPerVolume
						initialFreeEnergy = totalSum;
					...
					//	freeEnergyTotals.add(particles.meanFreeEnergy());
						meanPressures.add(particles.meanPressure()+dFRFE_dPhi); // P_Virial+P_Swelling
						pairEnergyTotalPerVolumes.add(pairEnergyTotalPerVolume);
						reservoirVolFracs.add(particles.reservoirVolFrac); //reservoirVolFrac 
					//	chemicalPotentialsList.add((4.*Math.PI/3.)*totalSum[i+1])

					//  chemPotList.add((4.*Math.PI/3.)*(freeEnergyPerVol[i+1]-freeEnergyPerVol[i-1])/dphi0/2.);

						initialFloryRehnerFreeEnergy = freeEnergyPerVolume; // set the value of the initialFLoryFreeEnergy
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergyLambdaEditedCopy.txt");
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, SumOfEnergies, pairFreeEnergy, IdealGasFreeEnergy, FloryRehnerFreeEnergy, reservoirVolFrac, meanPressure(), calculatedPressure()");
			bw1.newLine();		

			for (int i = 0; i < dryVolFracs.size(); i++) {
				bw1.write(dryVolFracs.get(i) + ", " + totalSums.get(i) + ", " + pairEnergyTotalPerVolumes.get(i) + ", " + freeEnergyIdealPerVolumes.get(i) + ", " + freeEnergyPerVolumes.get(i) + ", " + reservoirVolFracs.get(i) + ", " + meanPressures.get(i) + ", " + pairPressures.get(i));
				bw1.newLine();
			}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresLiquidLambdaEditedApp());
	}
}
