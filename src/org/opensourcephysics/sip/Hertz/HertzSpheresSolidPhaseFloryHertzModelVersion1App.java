/*
 * 
 * This code is an extension from the HertzSpheresSolidPhaseFloryHertzModelApp. 
 * In this code, I am only computing the Lindemann ratio, after accounting for the faceting of the microgels.
 */
...
public class HertzSpheresSolidPhaseFloryHertzModelVersion1App extends AbstractSimulation {
	...
	HertzSpheresNonLocalFacet_Copy particles = new HertzSpheresNonLocalFacet_Copy();
			...
			if (xLinkFrac < xLinkFracMax){
				...
				if (dryVolFrac < dryVolFracMax) {
					...
					if(lindemannParameter<0.15){
						...
						//control.println("Lindemann parameter: "+lindemannParameter +", dryVolFrac: "+dryVolFrac + ", x-link fraction: "+xLinkFrac);
	...
	public void writeData() {
		...
		try {
			...
			File outputFile = new File("data/Lindemann_Parameter_ISMC/NonLocalFacetStability"+particles.fileExtension+".txt");
			...
			// bw1.newLine();
			bw1.write("#phi0,		   phi,      		zeta, 	 	 	  lindemannParameter");
			bw1.newLine();

			for (int i = 1; i < dryVolFracs.size() - 1; i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				// bw1.write(roundedDryVolFrac + ", " + volumefractionList.get(i) + ", " + swellingRatioList.get(i) + ", " + reservoirVolFracList.get(i) + ", " + lindemannParameterList.get(i) + ", " + springConstantList.get(i));
				bw1.write(roundedDryVolFrac + ", " + volumefractionList.get(i) + ", " + reservoirVolFracList.get(i) + ", " + lindemannParameterList.get(i));
				bw1.newLine();
			}
			bw1.close();
		} catch (IOException e) {
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidPhaseFloryHertzModelVersion1App());
	}
}
