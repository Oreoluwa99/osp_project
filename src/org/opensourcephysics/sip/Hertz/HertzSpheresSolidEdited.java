...
public class HertzSpheresSolidEdited {
    ...
    public double pairEnergy[][], newPairEnergy[][], newPairEnergy1[][]; // pair energies of particles
    ...
    public double boltzmannFactorAccumulator, boltzmannFactor;
   ...
   public void initialize(String configuration) {
      ...
      virialAccumulator = 0;
      virialAccumulator1 = 0; // for lambda = 1 system
   ...
   public void setFCCpositions() {
      ...
      calculateTotalEnergy(lambda); // initial energy 
      initialEnergy = pairEnergyAccumulator1;// when lambda = 1
   }
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
            ...
            if(j != i) { // consider interactions with other particles
               ...
               r2 = xij*xij + yij*yij + zij*zij; // particle separation squared
	            ...
	            if (r2 < sigma*sigma){
                  ...
                  HertzEnergy1 = B1*Math.pow(1-r/sigma, 2.5);
                  newPairEnergy1[i][j] = HertzEnergy1;
		            pairEnergySum1 += HertzEnergy1;
	            }
            }
         ...
         if (lambda == 0){
            ...
            newEnergy = pairEnergySum1+(1-lambda)*springEnergy+totalF; // total energy after trial move
         }
         ...
         // System.out.println("Move accepted");
            ...
            for (int j = 0; j < N; ++j) { // update energies of other particles 
               if(j != i) {
                  energy[j] += newPairEnergy1[i][j]-pairEnergy1[i][j];
                  pairEnergy1[i][j] = newPairEnergy1[i][j];
                  pairEnergy1[j][i] = newPairEnergy1[i][j];
               }
            }
         }
      }
   ...
   public void calculateTotalEnergy(double lambda) {
      ...
      for(int i = 0; i < N; ++i) {
         ...
         for(int j = 0; j < N; ++j) {
            if(j != i) { // consider interactions with other particles
               ...
               r2 = xij*xij + yij*yij + zij*zij; // particle separation squared
	            ...
	            if (r2 < sigma*sigma){
                  r = Math.sqrt(r2);
                  // Hertz pair potential amplitude (scaled by a factor of scale*Young) for the lambda system
         ...
         if (lambda == 0){
            ...
            energy[i]=pairEnergySum1+(1-lambda)*springEnergy+totalF; // total energy of particle i for the lambda system
         }   
...
*/
   ...
   // System.out.println("initialEnergy-totalPairEnergy1 = " +(initialEnergy-totalPairEnergy1));
      if (lambda == 0){
         boltzmannFactor = Math.exp(initialEnergy-totalPairEnergy1);
   ...
   public double meanEnergy() {
      return energyAccumulator1/N/(double)(steps); // quantity <E>/N
   }
