 ...
 * ORIGINAL VERSION
 * -------------------------------------------------------------------------
 * This class implements a Monte Carlo simulation of nonionic microgels 
 * interacting via the Hertz elastic pair potential, and incorporates 
 * Flory-Rehner theory to calculate the free energy associated with the 
 * swelling of microgels. The simulation models the interactions between 
 * microgels, considering interpenetration, for calculating the Lindemann 
 * parameter and the free energies.
 * 
 * This is the correct code for the interpenetration algorithm and it works 
 * just fine. 
 * 
 * IMPORTANT DATES:
 * - December 30, 2024: Last confirmed date when this version ran correctly.  
 * - August 16, 2025: File name updated with "_original" suffix to mark this 
 *   as the baseline reference version before creating optimized variants. 
 *
 * -------------------------------------------------------------------------
 */
...
public class HertzInterpenetrationFreeEnergies_Original {
    ...
    public double volumeOfSolvent;
    public double mixF_FR, mixF_Interpenetration;
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
         ...
         oldFloryFR = elasticF + mixF; // total FR before the trial move for microgel i

         //System.out.println("oldFloryFR: "+oldFloryFR);
         ...
         for (int j = 0; j < N; j++) {
            if(j != i) { // consider interactions with other particles
               ...
               //Define phipi and phipj
               double phipi =1/Math.pow(a[i], 3);
               double phipj =1/Math.pow(a[j], 3);

               // Rewriting the expressions in terms of phipi and phipj
               double firstMixOverlap = (1-phipi-phipj)*Math.log(1-phipi-phipj);
               double secondMixOverlap = chi * (1-phipi-phipj) * (phipi + phipj);
               ...
               double secondMixNoOverlap = (1-phipj)*Math.log(1-phipj)  +  chi*(1-phipj ) * phipj ;
	            ...
	            if (r2 < sigma*sigma){    
                  ...
                  capVolSum = (capVolForI+capVolForJ); // the lens-shaped region

                  // // Calculate the overlap volume directly
                  // double R = Math.min(a[i], a[j]); // Use the smaller radius for calculation
                  // double overlapVol = (Math.PI * (4 * R + r) * Math.pow((2 * R - r), 2)) / 12;

                  mixF_FR = (capVolSum/volumeOfSolvent)*(firstMixOverlap+secondMixOverlap-firstMixNoOverlap-secondMixNoOverlap);
                  
                  newPairEnergy[i][j] = mixF_FR;

                  //System.out.println("r: " + r + " pairEnergy Between " + i + " and " + j + " is: " + newPairEnergy[i][j]);

                  mixF_Interpenetration += mixF_FR;

	            }
               else{ // microgels not interpenetrating
         ...
         // Flory-Rehner single-particle free energy (associated with swelling)
	      mixF = nMon*((a[i]*a[i]*a[i]-1)*Math.log(1-1/a[i]/a[i]/a[i])+chi*(1-1/a[i]/a[i]/a[i]));
	      elasticF = 1.5*nChains*(a[i]*a[i]-Math.log(a[i])-1);
	      newFloryFR = elasticF + mixF; // before the trial move
         ...
         if(Math.exp(-de) < Math.random()){ // Metropolis algorithm
            ...
            //System.out.println("Move Accepted and energy updated");
            ...
            energy[i] = mixF_Interpenetration; // update energy of moved particle
   ...
   public void calculateTotalEnergy(double lambda) {
      ...
      for(int i = 0; i < N; ++i) {
         ...
         energy[i] = mixF_Interpenetration; // total energy of particle i for the lambda system // because the change in energy is computed in the step method and the change has been used in the step method
         ...
         if (steps > delay){
            if ((steps-delay)%snapshotInterval==0){ // include configurations only after even number of intervals
               ...
               volFracAccumulator += calculateVolumeFraction(); // Accumulate the volume fraction
