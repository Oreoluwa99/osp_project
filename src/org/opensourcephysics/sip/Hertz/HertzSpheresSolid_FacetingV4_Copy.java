 ...
 */
 ...
 public class HertzSpheresSolid_FacetingV4_Copy {
     ...
     public double capVolSum, capVolForI, capVolForJ, capVolSumBeforeMove, capVolSumAfterMove;
     public double volOfMicrogel[];
     public double facetEnergy;
     public double capVolBeforeMove[], floryFRJBeforeMove[], floryFRJAfterMove[];
    ...
    */
    ...
    public void step() { // performs a trial move of every particle
       ...
       for (i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
         ...
         double floryFRIBeforeMove = mixF+elasticF;
          ...
          for (j = 0; j < N; j++) {
             if(j != i) { // consider interactions with other particles
                ...
                if (r2 < sigma*sigma){
                   ...
                   newPairEnergy[i][j] = HertzEnergy+capVolForI;
                   pairEnergySum+=HertzEnergy;
                   
                   capVolSum += capVolForI;
                }
             }
          } 
         ...
         mixF = newNMon*((a[i]*a[i]*a[i]-1)*Math.log(1-1/(a[i]*a[i]*a[i]))+chi*(1-1/(a[i]*a[i]*a[i])));
         elasticF = 1.5*newNMon*xLinkFrac*(a[i]*a[i]-Math.log(a[i])-1);
         double floryFRIAfterMove = mixF+elasticF;
         ...
         double dFlory = floryFRIAfterMove-floryFRIBeforeMove; // change in energy for microgel i
         ...
         de = (pairEnergySum-energy[i])+dFlory;
          ...
          //de = ((pairEnergySum+capVolSum)-energy[i])+(newFloryFR-oldFloryFR); // change in total energy due to trial move
