...
public class HertzSpheresSolidPhaseWithoutPBC {
   ...
   */
      ...
      for(int i = 0; i < N; ++i) {
         ...
         for(int j = 0; j < N; ++j) {
            if(j != i) { // consider interactions with other particles
               ...
               r2 = xij*xij + yij*yij + zij*zij; // particle separation squared
               if (lambda == 1){
                  displacementX = x[i] - x0[i];
                  displacementY = y[i] - y0[i];
                  displacementZ = z[i] - z0[i];               
               }
         ...
         if (lambda == 1){
            squaredDisplacement = Math.pow(displacementX, 2) + Math.pow(displacementY, 2) + Math.pow(displacementZ, 2);
         }
   ...
   public double meanSquareDisplacement(){
      return squaredDisplacement/N/numberOfConfigurations; // quantity <r>^2
   }
