...
public class HertzSpheresApp extends AbstractSimulation {
    public enum WriteModes{WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
    HertzSpheres particles = new HertzSpheres();
	...
	*/
	public void initialize() {
		...
		added = false;
		particles.dlambda = control.getDouble("The increment");
	...
	public void doStep() { // logical step in the HertzSpheres class	    	    
		...
		if (particles.steps == particles.stop){
			...
			if (dryVolFrac < dryVolFracMax){
				/*
				if (incrementDryVolFrac){
					//prints out the first loop from lambda = partiles.dlambda/2 through till it is >= 1
					...
					control.println("DryVolFrac = " + particles.dryVolFrac);
					...
					if (lambda == 1){
						totalSum = (EpairSum * particles.dlambda) + particles.meanFreeEnergy();
						control.println("totalSum = " + totalSum);

						// Update the array with the latest values of dryVolFrac and totalSum
					...
					EpairSum += (particles.meanPairEnergy()/particles.lambda);
					lambda += particles.dlambda; // increment the value of lambda
					particles.lambda = lambda;
					...
					if (lambda > 1){
						...
						this.initialize();
						return;
						//control.println("lambda for the fully interacting system = " + lambda);
					}
	...
	public void reset() {
		...
		control.setValue("The increment", 0.1);
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergy.txt");
			...
			bw1.newLine();
			bw1.write("Mean pressure PV/NkT: " + particles.meanPressure());
			bw1.newLine();
			bw1.newLine();
			bw1.write("Mean free energy per particle <F>/N [kT]: " + particles.meanFreeEnergy());
			bw1.newLine();
		
			for (int i = 0; i < dryVolFracs.size(); i++) {
        	bw1.write("DryVolFrac: " + dryVolFracs.get(i) + ", TotalSum: " + totalSums.get(i));
        	bw1.newLine();
    		}
			bw1.close();
		}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresApp());
	}
}
