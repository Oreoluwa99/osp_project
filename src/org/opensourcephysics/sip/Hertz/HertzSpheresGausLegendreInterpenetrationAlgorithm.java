/*
 * The major reason for this code is to account for the faceting of the microgels in the step() method
 * Formerly HertzSpheresSolidPhaseCopyFloryHertzModelVersion4.java
 * It is all set up for Faceting and works just fine.
 */
...
public class HertzSpheresGausLegendreInterpenetrationAlgorithm {
    ...
    public double newNMon[], cumulativeCapVol[], capVol[], cumulativeCapVolAfterMove[];
   ...
   public void initialize(String configuration) {
      ...
      overlappingBeforeMove = new boolean[N][N];
      overlappingAfterMove = new boolean[N][N];
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      double capVolForI, capVolSumBeforeMove=0, capVolSumAfterMove=0;
      double da2;
      ...
      double mixingFRBeforeMove, mixingFRAfterMove;
      double totalCapVolume;
      double totalOverlapVolume;
      double firstMix, secondMix;
      ...
      mixingFRBeforeMove = 0;

      for (i = 0; i < N; i++) { // one microgel 
         ...
         for (j = 0; j < N; j++) {
            ...
            if(j != i) { // consider interactions with other particles
               ...
               if (r2 < sigma*sigma){
                  ...
                  totalOverlapVolume = (capVolForI+capVolForJ);
               }
            ...
            mixingFRBeforeMove += (totalOverlapVolume/volumeOfSolvent)*(firstMixOverlap+secondMixOverlap-firstMixNoOverlap-secondMixNoOverlap);
         }
         ...
         for (j = 0; j < N; j++) {
               ...
               capVolSumAfterMove = 0;
	            ...
	            if (r2 < sigma*sigma){
                  ...
                  //B = scale*Young*nChains*Math.pow(sigma, 2.)*Math.sqrt(a[i]*a[j])/(Math.pow(a[i], 3.)+Math.pow(a[j], 3.)); 
		            B = 0; // for the interpenetration model
                  ...
                  // sum up the cap volume for the lens shaped region
                  capVolSumAfterMove = (capVolForI+capVolForJ);        
            ...
            mixingFRAfterMove += (capVolSumAfterMove/volumeOfSolvent)*(firstMixOverlap+secondMixOverlap-firstMixNoOverlap-secondMixNoOverlap);

         } 

         dFlory = mixingFRAfterMove-mixingFRBeforeMove; // change in energy for microgel i

         //System.out.println("dFlory: "+ dFlory);
         
         //de = (pairEnergySum-energy[i])+dFlory; //Edited by Oreoluwa 02-Sep-2024
         ...
         de = lambda*((pairEnergySum+mixingFRAfterMove)-energy[i])+(1-lambda)*pairPotentialCorrection+(newFloryFR-oldFloryFR); // change in total energy due to trial move
         //de = ((pairEnergySum+mixingFRAfterMove)-energy[i])+(newFloryFR-oldFloryFR); // change in total energy due to trial move
   ...
   public void calculateTotalEnergy(double lambda) {
      ...
      for(int i = 0; i < N; ++i) {
            ...
            if(j != i) { // consider interactions with other particles
               ...
               r2 = xij*xij + yij*yij + zij*zij; // particle separation squared
	            ...
	            if (r2 < sigma*sigma){
                  ...
                  double firstMixOverlap = (1-(1/Math.pow(a[i], 3))-(1/Math.pow(a[j], 3)))*Math.log(1-(1/Math.pow(a[i], 3))-(1/Math.pow(a[j], 3)));
                  double secondMixOverlap = chi * (1-(1/Math.pow(a[i], 3))-(1/Math.pow(a[j], 3))) * ((1/Math.pow(a[i], 3)) + (1/Math.pow(a[j], 3)));
                  double firstMixNoOverlap = (((1-(1/Math.pow(a[i], 3)))*Math.log(1-(1/Math.pow(a[i], 3)))) + (chi*(1-(1/Math.pow(a[i], 3))*Math.log(1-(1/Math.pow(a[i], 3))))));
                  double secondMixNoOverlap = (((1-(1/Math.pow(a[j], 3)))*Math.log(1-(1/Math.pow(a[j], 3)))) + (chi*(1-(1/Math.pow(a[j], 3))*Math.log(1-(1/Math.pow(a[j], 3))))));
                  ...
                  // Calculate cap volumes
                  double capVolForI = (Math.PI*Math.pow(hi, 2)*(3.0*a[i]-hi))/3.0;
                  double capVolForJ = (Math.PI*Math.pow(hj, 2)*(3.0*a[j]-hj))/3.0;
 
                  // sum up the cap volume for the lens shaped region
                  double capVolSumAfterMove = (capVolForI+capVolForJ);        

                  // mixingFRAfterMove = capVolSumAfterMove/volumeOfSolvent*(firstMix-secondMix);
                  mix_F += (capVolSumAfterMove/volumeOfSolvent)*(firstMixOverlap+secondMixOverlap-firstMixNoOverlap-secondMixNoOverlap);
         ...
         energy[i] = pairEnergySum + mix_F; // total energy of particle i for the lambda system // because the change in energy is computed in the step method and the change has been used in the step method
   ...
   public double meanRadius() { // mean radius [units of dry radius]
    ...
    public double reservoirSwellingRatio(double nMon, double nChains, double chi) {
      // Create the FloryRehnerPressure function
      FloryRehnerPressure f = new FloryRehnerPressure(nMon, nChains, chi);
      
      // Define the bounds and epsilon
      double xleft = 1.1;
      double xright = 15.0;
      double epsilon = 1.e-06;
  
      // Pass the function 'f' into the Root.bisection method
   ...
   class FloryRehnerPressure implements Function {
      ...
      // Implement the evaluate method required by org.opensourcephysics.numerics.Function
      @Override
      public double evaluate(double x) {
          double pMixing = nMon * (x*x*x * Math.log(1. - 1./(x*x*x)) + 1. + chi/(x*x*x));
          double pElastic = nChains * (x*x - 0.5);
          return pMixing + pElastic;
      }
  }
