...
public class HertzSpheresNonLocalFacet_CopyApp extends AbstractSimulation {
	public enum WriteModes {WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
	// HertzSpheresSolidPhase particles = new HertzSpheresSolidPhase(); 			
	//HertzSpheresSecondInterpenetration_Copy particles = new HertzSpheresSecondInterpenetration_Copy();
	...
	double initialVolume, latestVolume;
	double floryPressure, pairPressure;
	...
	public void doStep() { // logical step in the HertzSpheres class
		...
		if (particles.steps == particles.stop) {
			...
			if (dryVolFrac < dryVolFracMax) {
				...
				//control.println("phi: "+(particles.calculateVolumeFraction()));
	...
	public void reset() {
		...
		control.setValue("File extension", "10");
	...
	public void writeData() {
		...
		try {
			//File outputFile = new File("data/Lindemann_Parameter/StabilityNonLocalFacet"+particles.fileExtension+".txt");
			//File outputFile = new File("data/Lindemann_Parameter/StabilityData"+particles.fileExtension+".txt");
			 File outputFile = new File("data/Lindemann_Parameter_ISMC/StabilityNonLocalFacet"+particles.fileExtension+".txt");
			//File outputFile = new File("data/Lindemann_Parameter_ISMC/StabilityInterpenetration"+particles.fileExtension+".txt");
			...
			bw1.newLine();
			bw1.write("#Minimum x-link fraction: " + particles.xLinkFrac);
			bw1.newLine();
			bw1.write("#Maximum x-link fraction: " + particles.xLinkFrac);
			bw1.newLine();
			...
			bw1.newLine();
			bw1.write("#phi0,		      <alpha>,      		zeta, 	 	 	  lindemannParameter");
			bw1.newLine();

			for (int i = 1; i < dryVolFracs.size() - 1; i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + swellingRatioList.get(i) + ", " + reservoirVolFracList.get(i) + ", " + lindemannParameterList.get(i));
				bw1.newLine();
			}
			bw1.close();
		} catch (IOException e) {
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresNonLocalFacet_CopyApp());
	}
}
