...
public class HertzSpheresSecondFacetingAlgorithm {
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
         ...
         // The Flory-Rehner free energy due to the faceting
         double newNMon = nMon*(volOfMicrogel[i]-capVolSum)/volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
         double mixF_facet = newNMon*((a[i]*a[i]*a[i]-1)*Math.log(1-1/(a[i]*a[i]*a[i]))+chi*(1-1/(a[i]*a[i]*a[i])));
         double elasticF_facet = 1.5*newNMon*xLinkFrac*(a[i]*a[i]-Math.log(a[i])-1);
         facetEnergy = mixF_facet+elasticF_facet;
         
         de = ((pairEnergySum+facetEnergy)-energy[i])+(newFloryFR-oldFloryFR); // change in total energy due to trial move
   ...
   public void calculateTotalEnergy(double lambda) {
      ...
      for(int i = 0; i < N; ++i) {
         ...
         // The Flory-Rehner free energy due to faceting
         double newNMon = nMon*(volOfMicrogel[i]-capVolSum)/volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
         double mixF_facet = newNMon*((a[i]*a[i]*a[i]-1)*Math.log(1-1/(a[i]*a[i]*a[i]))+chi*(1-1/(a[i]*a[i]*a[i])));
         double elasticF_facet = 1.5*newNMon*xLinkFrac*(a[i]*a[i]-Math.log(a[i])-1);
         facetEnergy = mixF_facet+elasticF_facet;
         ...
         energy[i] = pairEnergySum + facetEnergy; 
