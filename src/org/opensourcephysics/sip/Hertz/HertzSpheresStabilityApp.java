/*
 * This code is set up mainly for computing the stability curve for the 
 * Faceting algorithms (non local and local), alongside the interpenetration algorithm. 
...
import org.opensourcephysics.sip.Hertz.HertzSpheresNonLocalFacet_Copy;
...
public class HertzSpheresStabilityApp extends AbstractSimulation {
	...
	public void reset() {
		...
		// control.setValue("x-Link increment", 0.000003); // for about 23 data points
		...
		control.setValue("Stop", 150000); // steps after which statistics collection stops
	...
	public void writeData() {
		...
		try {
			...
			File dir = new File("data/Comprehensive_Report_Data/Non_Local_Facet/");
			if (!dir.exists()) {
				if (dir.mkdirs()) {
			...
			File outputFile = new File(dir, "StabilityNon_Local_Facet" + particles.fileExtension + ".txt");
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresStabilityApp());
	}
}
