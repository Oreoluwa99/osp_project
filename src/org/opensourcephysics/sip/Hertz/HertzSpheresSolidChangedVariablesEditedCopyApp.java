 ...
 */
 ...
 public class HertzSpheresSolidChangedVariablesEditedCopyApp extends AbstractSimulation {
	...
	double x;
	double uLattice, delta1, delta1PerVolume, totalFreeEnergy, totalFreeEnergyPerVolume;
	double totalSumOfEnergiesPerVolume, floryRehnerfreeEnergyPerVolume;
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("The coupling constant = "+lambda);
				...
				if (x<upperLimit){
					...
					if (lambda == 1){
						control.println("Lambda at 1 = "+lambda);
					...
					if (lambda == 0){
						...
						control.println("delta1PerVolume = "+delta1PerVolume);
						...
						control.println("totalSumOfEnergyPerVolume = " + totalSumOfEnergiesPerVolume);
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergySolidPhaseCopy.txt");
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, delta2, totalFreeEnergy");
			bw1.newLine();	

			for (int i = 0; i < dryVolFracs.size(); i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + delta2PerVolumeList.get(i) + ", " + totalSumOfEnergiesList.get(i));
				bw1.newLine();
			} 
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidChangedVariablesEditedCopyApp());
	}
}
