...
public class HertzSpheresLiquidDensityCopyApp extends AbstractSimulation {
	...
	double totalSum;
	double totalVol;
	double floryRehnerFR, referenceFR;
	double gaussPoint, gaussWeight;
	double path1FRAccumulator = 0, path2FRAccumulator = 0, path3FRAccumulator = 0;
	double n, pairPressure, calculatedPressure;
	double path3FRPerN, path3FRPerPerVol;
	double deltaF2PerN, deltaF2PerVol;
	double path2FRPerN, path2FRPerPerVol;
	double initialfloryFperVol, initialpairEperVol;
	double stirlingApprox, deltaF1PerVol, totalFreeEnergies;
	...
	double youngStart, young;
	boolean incrementDryVolFrac = true;
	boolean flag = false;
	boolean resetLambda = true;
	boolean isFirstIteration;
	boolean resetYoung = true;
	boolean firstResidualPart;
	boolean gaussLegendreWeight=true, gaussLegendrePoint=true;
	boolean path1=true, path2=true, path3=true;
	...
	List<Double> referenceFRList = new ArrayList<>();
	...
	List<Double> deltaF2List = new ArrayList<>();
	List<Double> deltaF3PerVolList = new ArrayList<>();
	...
	*/
	public void initialize() {
		...
		particles.highB = control.getDouble("Highest Young's calibration");
		particles.dYoung = control.getDouble("Young's Modulus Increment");
        ...
        youngStart = control.getDouble("Young's calibration"); // 10-1000
		...
		*/
		...
		if (resetYoung){
			...
			young = (gaussPoint+1)/2; // comes from changing limits in the integration
			particles.Young = young;
			resetYoung = false;
		}
	...
	public void doStep() { // logical step in the HertzSpheres class	    	    
		...
		if (particles.steps == particles.stop){
			...
			if (dryVolFrac < dryVolFracMax){
					...
					if (path1){ // the path from the reference system to a state with higher temperature (1st isochoric path)
						...
						referenceFR = (0.75/Math.PI)*dryVolFrac*(Math.log(density)-1.0)+stirlingApprox; // ideal gas free energy contribution
						referenceFRList.add(referenceFR); // update the list
						path1 = false;
					
					}

					if (path2){ // the isotherm path to the free energy
						// increment the pairEnergy 
						path2FRAccumulator += (particles.meanPressure()-1)*2/n; // n is a double that increases by a factor of 2
						path2 = false;
					}

					if (path3){ // the second isochoric path to the free energy
						path3FRAccumulator += 0.5*gaussWeight*(particles.highB-particles.Young)*particles.meanPairEnergy(); // Accumulate the pairEnergies
						//particles.lowB+=particles.dYoung;
						young += particles.dYoung;
						particles.Young = young;
						...
						control.println("path3FRAccumulator: "+path3FRAccumulator);
						control.println("DryVolFrac: " + dryVolFrac);
						control.println("particles.Young: "+particles.Young);

						/*
					 	 * Does a check on the lists
					 	 */
						if (pointIteration < gaussLegendrePoints.size() && weightIteration < gaussLegendreWeights.size()) {				
							...
							path3FRPerN = path3FRAccumulator*particles.dYoung; 
							path3FRPerPerVol = path3FRPerN * (particles.N/particles.totalVol); // over the entire volume

							path3FRAccumulator = 0; // reset the accumulator
							path3 = false;
					...
					// the isotherm path
					deltaF2PerVol = (path2FRAccumulator*particles.N)/particles.totalVol; // the total pair energy per volume
					control.println("deltaF2PerVol: "+deltaF2PerVol);

					// the isochoric path
					deltaF3PerVol = path3FRPerPerVol;
					deltaF3PerVolList.add(deltaF3PerVol); 

					floryRehnerFR = (particles.meanFreeEnergy() * particles.N)/particles.totalVol; // flory-rehner free energy per volume
					control.println("fFlory/Vol: "+floryRehnerFR);
					
					// the 3 paths to the free energy + Flory-Rehner contribution
					totalFR = referenceFR+deltaF2PerVol+deltaF3PerVol+floryRehnerFR;
					control.println("totalFR: "+totalFR);

					nnDistance = (1/Math.sqrt(2)) * Math.pow((4/density), 1/3.0); //nearest negibour distance
					...
					totalSums.add(totalFR);
					dryVolFracs.add(dryVolFrac); 
					totalVolList.add(particles.totalVol);
					pairEperVolList.add(deltaF1PerVol);
					floryFperVolList.add(floryRehnerFR);
					...
					pointIteration = 0; //reset the iterations
					weightIteration = 0;	
					gaussPoint = gaussLegendrePoints.get(pointIteration); // the Gauss-Legendre point
					gaussWeight = gaussLegendreWeights.get(weightIteration); // the Gauss-Legendre weight
					young = (gaussPoint+1)/2; // reset the young's modulus
					particles.Young = young;
	...
	public void reset() {
		...
		control.setValue("Highest Young's calibration", 10); // the desired Young's Modulus
		control.setValue("Young's Modulus Increment", 0.01); // the Young's Modulus increment
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresLiquidDensityCopyApp());
	}
}
