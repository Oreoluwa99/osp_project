 ...
 */
 ...
 public class HertzSpheresSolidEinsteinCrystalMethodApp extends AbstractSimulation {
...
//	HertzSpheresSolidEinsteinCrystalCopy particles = new HertzSpheresSolidEinsteinCrystalCopy(); // without equilibration
			...
			//	control.println("Coupling constant = "+lambda);
				...
				if (x<upperLimit){
					...
					//	control.println("fPair = " + fPair);
						...
						dFRFE_dPhi = (floryFEperVol-initialFloryFreeEnergy)/(2*particles.dphi); // change in free energy by dryVolFrac change
						dFPair_dPhi = (fPairPerVol - initialPairFreeEnergy)/(2*particles.dphi);
						...
						pairPressuresList.add(pairPContribution); // (PV/NKT)pair
					...
					//	latestVolume = totalVolList.get(totalVolList.size()-1);

					//	freeEnergyChange = (totalFreeEnergy-initialFreeEnergy)*(particles.totalVol/particles.N);
					//	volumeChange = latestVolume-initialVolume;

					//	calculatedPressure = -freeEnergyChange/volumeChange; //compute the calculated pressure
					//	calculatedPressures.add(calculatedPressure);

						initialFreeEnergy = totalFreeEnergy;
						initialFloryFreeEnergy = floryFEperVol; // update the initial flory-rehner energy
						initialPairFreeEnergy = fPairPerVol; // update the initial pair free energy
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidEinsteinCrystalMethodApp());
	}
}
