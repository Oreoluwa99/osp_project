...
public class HertzSpheresFacetAndInterpenetrationApp extends AbstractSimulation {
	...
	int dataPoints; // number of data points to be collected
	int maxDataPoints; // maximum number of data points to be collected
	int weightIteration = 0, pointIteration = 0; // set and reset the iterations
	ElementSphere nanoSphere[];
	boolean added = false; // whether the particles have been added to the display3d frame
	boolean structure; // whether to calculate structure or not
	RDF rdf; // radial distribution function
	SSF ssf; // static structure factor
	double dryVolFracStart, dryVolFracMax, dryVolFrac; // the starting and maximum dry volume fraction, and the current dry volume fraction
	double lambda, totalVol; // the coupling constant and total volume of the system
	boolean setLambda = true;
	boolean incrementDryVolFrac = true, incrementXLinkFrac=true; // increment dry volume fraction and cross-link fraction
	double floryFEperVol; // Flory-Rehner free energy per volume
	double initialVolume, latestVolume; // initial and latest volume of the microgel
	double floryPressure, pairPressure; // Flory-Rehner and pair pressures
	double density, nnDistance; // density of particles and nearest neighbor distance
	double deltaFPerVol; // change in free energy per volume
	double springConstant; // the spring constant
	double lindemannParameter; // the lindemann parameter
	double xLinkFracStart, xLinkFracMax; // the cross-link fraction start and max values
	double dxLink; // the cross-link fraction increment
	double xLinkFrac; // the cross-link fraction
	...
	public void reset() {
		...
		control.setValue("DryVolFracStart", 0.0019);
	...
	public void writeData() {
		...
		try {
			...
			File outputFile = new File("data/ssf_data/facet_data"+particles.fileExtension+".txt");
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresFacetAndInterpenetrationApp());
	}
}
