...
public class HertzSpheresLiquidAppEdited extends AbstractSimulation {
    public enum WriteModes{WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
    HertzSpheresLiquid particles = new HertzSpheresLiquid();
	...
	double totalPairFreeEnergy, totalPairFreeEnergyPerVolume;
	...
	*/
		...
		if (particles.steps == particles.stop){
			...
			if (dryVolFrac < dryVolFracMax){
				...
				//	control.println("<Epair>/lambda = " + decimalFormat.format(particles.meanPairEnergy()/particles.lambda));
					...
					if (lambda == 0){
						System.out.println("The value of lambda at lambda = 0 "+lambda);

						control.println("lambda = "+lambda);
						totalPairFreeEnergy = pairEnergyTotal + particles.meanPairEnergy();
						totalPairFreeEnergyPerVolume = (totalPairFreeEnergy*particles.N)/particles.totalVol;
						control.println("TotalPairEnergy = "+totalPairFreeEnergyPerVolume);

						freeEnergyIdealPerVolume = (0.75/Math.PI)*dryVolFrac*(Math.log(dryVolFrac)-1.0);
						stirlingApproximation = (0.75/Math.PI)*dryVolFrac*Math.log(2*Math.PI*particles.N)/(2*particles.N);

						totalSum = (totalPairFreeEnergyPerVolume + freeEnergyPerVolume + freeEnergyIdealPerVolume+stirlingApproximation);
						control.println("totalSum = " + totalSum);

						pairEnergyTotalPerVolumes.add(totalPairFreeEnergyPerVolume);
						totalSums.add(totalSum);
					...
					//	freeEnergyTotal = 0; //reset freeEnergyTotal to zero
						...
						particles.dryVolFrac = dryVolFrac;
						lambda = particles.dlambda*0.5; //reset lambda to dlambda/2
						particles.lambda = lambda;
					...
					//	freeEnergyTotal += (particles.meanFreeEnergy());
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergyLambda.txt");
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, SumOfEnergies, pairFreeEnergy");
			bw1.newLine();		

			for (int i = 0; i < dryVolFracs.size(); i++) {
				bw1.write(dryVolFracs.get(i) + ", " + totalSums.get(i) + ", " + pairEnergyTotalPerVolumes.get(i));
				bw1.newLine();
			}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresLiquidAppEdited());
	}
}
