 ...
 */
 ...
 public class HertzSpheresSolidApp extends AbstractSimulation {
    public enum WriteModes{WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
	HertzSpheresCopy particles = new HertzSpheresCopy();
...
//	HertzSpheresSolidPhaseTrialCode particles = new HertzSpheresSolidPhaseTrialCode();
	...
	double pairPressure;
	double pairEnergyTotalPerVolumeChange;
	double latestpairEnergyTotalPerVolume;
	double pairEnergyTotalPerVolumeChangebyDryVolFracChange;
	...
	List<Double> einsteinFreeEnergyPerVolumes = new ArrayList<>();
	...
	public void doStep() { // logical step in the HertzSpheres class	    	    
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("<E_pair>/N = " + decimalFormat.format(particles.meanPairEnergy1()));
			//	control.println("Mean Pair Energies = " + particles.einsteinMeanPairEnergy());
				control.println("<F>/N = " + decimalFormat.format(particles.meanFreeEnergy()));
				if (lambda == 1){
					totalVol = particles.totalVol;

					// the free energy difference 
					freeEnergyDifference = (particles.calculateFreeEnergyDifference()*particles.N)/(double)(totalVol);
					control.println("deltaA1 = " + freeEnergyDifference);
					initalEnergy = (particles.initialEnergy*particles.N)/(double)totalVol;
					totalPairEnergy = totalEinsteinPotential*particles.dlambda;
					totalPairEnergyPerVolume = (totalPairEnergy * particles.N)/totalVol;
					...
					totalSumOfEnergies = totalPairEnergyPerVolume+einsteinFreeEnergyPerVolume+floryRehnerfreeEnergyPerVolume;
					control.println("The total sum of energies = " + totalSumOfEnergies);

					System.out.println("The pairEnergyAccumulator1 = " + particles.pairEnergyAccumulator1/(double)(particles.steps)/particles.N);
					System.out.println("The springEnergyAccumulator = " + particles.springEnergyAccumulator/(double)(particles.steps)/particles.N);
					...
					*/
					...
					einsteinFreeEnergyPerVolumes.add(einsteinFreeEnergyPerVolume);
					...
					totalEinsteinPotential = 0; //reset the totalEinsteinPotential to zero
				...
				if (lambda < 1){
					totalEinsteinPotential += particles.einsteinMeanPairEnergy(); //accumulate the einstein potentials
					control.println("The meanPairEnergy for the Einstein crystal = " + totalEinsteinPotential);
	...
	public void writeData(){
		...
		try {
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, initialEnergy, EnergyDifferenceBetweenIdealandInteracting");
			bw1.newLine();	

			for (int i = 0; i < dryVolFracs.size(); i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + initialEnergies.get(i) + ", " + freeEnergyDifferences.get(i));
				bw1.newLine();
			}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidApp());
	}
}
