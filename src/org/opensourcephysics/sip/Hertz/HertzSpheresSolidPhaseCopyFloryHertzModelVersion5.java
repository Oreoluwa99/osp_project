/*
 * We are taking into account the faceting of the microgels.
 * In this version of the code, I am only computing the facet volume for a single microgel i
 * 
 */
...
public class HertzSpheresSolidPhaseCopyFloryHertzModelVersion5 {
    ...
    public double boltzmannFactorAccumulator, boltzmannFactor, squaredDisplacementAccumulator, capToGelVolAccumulator;
    ...
    public boolean overlappingBeforeMove[][];
    public boolean overlappingAfterMove[][];
    public double FRforJBeforeMoveOverlap[];
    public double FRforJBeforeMoveNoOverlap[];
    public double FRforIBeforeMoveOverlap[];
    public double FRiBeforeMoveNoOverlap;
    public double FRforIBeforeMoveNoOverlap[];
    public double FRforJAfterMoveOverlap[];
    public double FRforJAfterMoveNoOverlap[];
    public double FRforIAfterMoveNoOverlap[];
    public double FRforIAfterMoveOverlap[];
   ...
   public void initialize(String configuration) {
      ...
      overlappingBeforeMove = new boolean[N+1][N+1];
      overlappingAfterMove = new boolean[N+1][N+1];
      FRforJBeforeMoveOverlap = new double[N];
      FRforJBeforeMoveNoOverlap = new double[N];
      FRforIBeforeMoveOverlap = new double[N];
      FRforIBeforeMoveNoOverlap = new double[N];
      FRforJAfterMoveNoOverlap = new double[N];
      FRforIAfterMoveNoOverlap = new double[N];
      FRforIAfterMoveOverlap = new double[N];
      ...
      volOfMicrogel = new double[N]; //volume of the fully swollen microgel
      newNMon = new double[N]; // the new number of monomers
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      double facetVolAccumulator = 0, capVolSum, capToGelVolSum=0;
      double amin, capVol, capToGelVol;
      double capVolSumForI, capVolSumForJ;
      double capVolForI, capVolForJ;
      double FRjBeforeMoveOverlap, FRjBeforeMoveNoOverlap;
      int i, j;
      double FRiBeforeMoveOverlap;
      double totalFRBeforeMove;
      double FRjAfterMoveOverlap, FRjAfterMoveNoOverlap;
      double totalFRAfterMove;
      double FRiAfterMoveOverlap, FRiAfterMoveNoOverlap;


      for (i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
          // Calculate the volume of the fully swollen microgel
         volOfMicrogel[i] = (4/3)*Math.PI*Math.pow(a[i], 3); 
         ...
         for (j = 0; j < N; j++) {
            ...
            //volume of the fully swollen microgel
            volOfMicrogel[j] = (4/3)*Math.PI*Math.pow(a[j], 3); 
            ...
            if(j != i) { // consider interactions with other particles
               ...
               if (r2 < sigma * sigma) {
                  ...
                  double hi = a[i] - (Math.pow(a[i], 2) - Math.pow(a[j], 2) + r2) / (2 * r);
                  double hj = a[j] - (Math.pow(a[j], 2) - Math.pow(a[i], 2) + r2) / (2 * r);

                  // Compute cap volumes
                  capVolForI = (Math.PI * Math.pow(hi, 2) * (3 * a[i] - hi)) / 3;
                  capVolForJ = (Math.PI * Math.pow(hj, 2) * (3 * a[j] - hj)) / 3;

                  //sum up the cap volumes
                  capVolSumForI += capVolForI;
                  capVolSumForJ += capVolForJ;

                  // Mark as overlapping
                  overlappingBeforeMove[i][j] = true;
                  overlappingBeforeMove[j][i] = true;

              }

               else{
                  // Mark as not overlapping
                  overlappingBeforeMove[i][j] = false;
                  overlappingBeforeMove[j][i] = false;
               }

               //System.out.println("overlappingBeforeMove[" + i + "][" + j + "]: " + overlappingBeforeMove[i][j]);

               // Calculate Flory-Rehner free energy for each particle interaction
               if (overlappingBeforeMove[i][j]) {
                  //compute the new number of monomers
                  double newNMon = nMon * (volOfMicrogel[j] - capVolSumForJ) / volOfMicrogel[j];
                  mixF = newNMon * ((a[j]*a[j]*a[j]-1)*Math.log(1-1/(a[j]*a[j]*a[j]))+chi*(1-1/(a[j]*a[j]*a[j])));
                  elasticF = 1.5 * newNMon * xLinkFrac * (a[j]*a[j]-Math.log(a[j])-1);
                  FRjBeforeMoveOverlap = elasticF + mixF;
                  FRforJBeforeMoveOverlap[j] = FRjBeforeMoveOverlap;
               } 
                  ...
                  newNMon[j] = nMon*(volOfMicrogel[j]-capVolSumForJ)/volOfMicrogel[j];
                  mixF = newNMon[j]*((a[j]*a[j]*a[j]-1)*Math.log(1-1/(a[j]*a[j]*a[j]))+chi*(1-1/(a[j]*a[j]*a[j])));
                  FRjBeforeMoveNoOverlap = elasticF + mixF;
                  FRforJBeforeMoveNoOverlap[j] = FRjBeforeMoveNoOverlap;

                  //mixF = nMon*((a[j]*a[j]*a[j]-1)*Math.log(1-1/(a[j]*a[j]*a[j]))+chi*(1-1/(a[j]*a[j]*a[j])));
                  //elasticF = 1.5 * nMon * xLinkFrac * (a[j]*a[j]-Math.log(a[j])-1);
                  //FRjBeforeMoveNoOverlap = elasticF + mixF;
                  //FRforJBeforeMoveNoOverlap[j] = FRjBeforeMoveNoOverlap;
               }
         ...
         for (j = 0; j < N; j++) {
            if(j!=i){
               // compute the Flory-Rehner free energy for the i microgel
               //if(!isFirstIteration){
                  if(capVolSumForI>0){ 
                     double newNMon = nMon * (volOfMicrogel[i] - capVolSumForI) / volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
                     mixF = newNMon * ((a[j]*a[j]*a[j]-1)*Math.log(1-1/(a[j]*a[j]*a[j]))+chi*(1-1/(a[j]*a[j]*a[j])));
                     elasticF = 1.5 * newNMon * xLinkFrac * (a[i]*a[i]-Math.log(a[i])-1);
                     FRiBeforeMoveOverlap = elasticF + mixF; // before the trial move
                     //System.out.println("FRAfterMoveOverlap: "+FRiAfterMoveOverlap);
                     FRforIBeforeMoveOverlap[i] = FRiBeforeMoveOverlap;   
                  }
                  else{
                     ...
                     FRiBeforeMoveNoOverlap = elasticF + mixF; // before the trial move
                     //System.out.println("FRiAfterMoveNoOverlap: "+FRiBeforeMoveNoOverlap);
                     FRforIBeforeMoveNoOverlap[i] = FRiBeforeMoveNoOverlap;
                  }
                  isFirstIteration = true;
               //}
            }

            // total flory-rehner free energy for both microgels if they were overlapping
            if(overlappingBeforeMove[i][j]){
               totalFRBeforeMove += FRforIBeforeMoveOverlap[i]+FRforJBeforeMoveOverlap[j]; //sum of the Flory-Rehner free energies
            }
            // total flory-rehner free energy for both microgels if they are not overlapping
            else{
               totalFRBeforeMove += FRforIBeforeMoveNoOverlap[i]+FRforJBeforeMoveNoOverlap[j]; 
            }
            ...
            if(j != i) { // consider interactions with other particles
               ...
               r2 = xij*xij + yij*yij + zij*zij; // particle separation squared
	            ...
	            if (r2 < sigma*sigma){
                  ...
                  System.out.println("capVolForJ: " + capVolForJ);

                  // sum up the cap volume for microgel i
                  capVolSumForI += capVolForI;

                  // mark as overlapping
                  overlappingAfterMove[i][j] = true;
                  //overlappingAfterMove[j][i] = true;
                  
	            }

               else{
                  // mark as not overlapping
                  overlappingAfterMove[i][j] = false;
                  //overlappingAfterMove[j][i] = false;
               }

               if (overlappingAfterMove[i][j]){
                  double newNMon = nMon * (volOfMicrogel[j] - capVolForJ) / volOfMicrogel[j]; // number of Monomers to compute the new FR free energy
                  // Flory-Rehner single-particle free energy (associated with swelling)
                  mixF = newNMon * ((a[j] * a[j] * a[j] - 1) * Math.log(1 - 1 / (a[j] * a[j] * a[j])) + chi * (1 - 1 / (a[j] * a[j] * a[j])));
                  elasticF = 1.5 * newNMon * xLinkFrac * (a[j] * a[j] - Math.log(a[j]) - 1);
                  FRjAfterMoveOverlap = elasticF + mixF;

                  FRforJAfterMoveOverlap[j] = FRjAfterMoveOverlap;
                  ...
                  elasticF = 1.5 * nMon * xLinkFrac * (a[j] * a[j] - Math.log(a[j]) - 1);
                  FRiBeforeMoveNoOverlap = elasticF + mixF; // before the trial move
                  FRjAfterMoveNoOverlap = elasticF + mixF;

                  FRforJAfterMoveNoOverlap[j] = FRjAfterMoveNoOverlap;
               }
            }
         } 
         ...
         for (j = 0; j < N; j++) {
            if(j != i){
               //if(!isSecondIteration){
                  if(capVolSumForI>0){
                     // the new number of monomers for microgel i
                     double newNMon = nMon * (volOfMicrogel[i] - capVolSumForI) / volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
                     // Flory-Rehner single-particle free energy (associated with swelling)
                     mixF = newNMon * ((a[i] * a[i] * a[i] - 1) * Math.log(1 - 1 / (a[i] * a[i] * a[i])) + chi * (1 - 1 / (a[i] * a[i] * a[i])));
                     elasticF = 1.5 * newNMon * xLinkFrac * (a[i] * a[i] - Math.log(a[i]) - 1);
                     FRiAfterMoveOverlap = elasticF + mixF; // after the trial move
                     //System.out.println("FRiAfterMoveOverlap: "+FRiAfterMoveOverlap);
                     FRforIAfterMoveOverlap[i] = FRiAfterMoveOverlap;
                  }
                     ...
                     mixF = nMon * ((a[i] * a[i] * a[i] - 1) * Math.log(1 - 1 / (a[i] * a[i] * a[i])) + chi * (1 - 1 / (a[i] * a[i] * a[i])));
                     elasticF = 1.5 * nMon * xLinkFrac * (a[i] * a[i] - Math.log(a[i]) - 1);
                     FRiAfterMoveNoOverlap = elasticF + mixF; // after the trial move
                     //System.out.println("FRiAfterMoveNoOverlap: "+FRiAfterMoveNoOverlap);
                     //FRiBeforeMoveOverlap = FRiBeforeMoveNoOverlap;
                     FRforIAfterMoveNoOverlap[i] = FRiAfterMoveNoOverlap;
                  }
               ...
               // total flory-rehner free energy for both microgels if they were overlapping
               if(overlappingAfterMove[i][j]){ 
                  totalFRAfterMove += FRforIAfterMoveOverlap[i]+FRforJAfterMoveOverlap[j];
               }
               // if they were not overlapping
               else{
                  totalFRAfterMove += FRforIAfterMoveNoOverlap[i]+FRforJAfterMoveNoOverlap[j];
               }
            }
         }

         double dFlory = totalFRAfterMove-totalFRBeforeMove;

         de = lambda*(pairEnergySum-energy[i])+(1-lambda)*(pairPotentialCorrection)+dFlory;
      ...
      if (steps > delay){
         if ((steps-delay)%snapshotInterval==0){ // include configurations only after even number of intervals
            ...
            capToGelVolAccumulator += capToGelVolSum;
         }
      }
   ...
   public double meanCapToGelVol(){
      return capToGelVolAccumulator/N/numberOfConfigurations; // quantity Vc/Vm
   }
