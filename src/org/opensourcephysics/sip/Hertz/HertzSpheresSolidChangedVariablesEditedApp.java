 ...
 */
 ...
 public class HertzSpheresSolidChangedVariablesEditedApp extends AbstractSimulation {
    public enum WriteModes{WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
	HertzSpheresSolidEinsteinCrystal particles = new HertzSpheresSolidEinsteinCrystal();
	...
	double delta2Accumulator = 0, delta2;
	double delta2PerVolume;
	double upperLimit, lowerLimit, xIncrement;
	double x;
	double boltzmannFactorAccumulatorPerSteps, boltzmannFactorPerN, delta1, delta1PerVolume, totalFreeEnergy, totalFreeEnergyPerVolume, totalSumOfEnergiesPerVolume; 
	double initialEnergy, springEnergySumAccumulator, floryRehnerfreeEnergyPerVolume, a0, a0overN, a0overNPerVolume, uLattice;


	List<Double> dryVolFracs = new ArrayList<>();
	List<Double> delta2List = new ArrayList<>();
	List<Double> delta2PerVolumeList = new ArrayList<>();
	...
	//	upperLimit = control.getDouble("upper limit");
		...
		/*
		 * set the value of x and lambda from changing the 
		 * variables in the integration
		 */
		
		if (setLambda){
			x = lowerLimit + 0.5*xIncrement; // set the value of x
	...
	public void doStep() { // logical step in the HertzSpheres class	    	    
		...
		if (particles.steps == particles.stop){
			...
			//	control.println("The coupling constant = "+lambda);
				control.println("The value of x = "+x);
				if (x<upperLimit){
					// 	accumulate delta2 
					delta2Accumulator += (particles.springEnergyAccumulator*Math.exp(x))/particles.springConstant;
					lambda = (Math.exp(x)-Math.exp(3.5))/particles.springConstant;
					particles.lambda = lambda;
					...
					if (x>upperLimit){
						/* calculate delta 2 */
						delta2 = -(delta2Accumulator*xIncrement)/(double)particles.steps/particles.N;
						delta2PerVolume = (delta2*particles.N)/particles.totalVol;
						control.println("delta2 = " + delta2);
						
						/* update the list that contains delta2 and the dryVolFrac */
						dryVolFracs.add(dryVolFrac);
						delta2PerVolumeList.add(delta2PerVolume);
						...
						particles.dryVolFrac = dryVolFrac;
						delta2Accumulator = 0; // reset delta2Accumulator to 0
					...
					if (lambda==1){
						...
						delta2 = -(springEnergySumAccumulator*particles.dlambda)/(double)particles.steps/particles.N;
						delta2PerVolume = (delta2*particles.N)/particles.totalVol;
					//	control.println("delta2 = " + delta2);
						
						/* calculate the floryRehner free energy at lambda = 1 */
						floryRehnerfreeEnergyPerVolume = (particles.meanFreeEnergy()*particles.N)/particles.totalVol; 
					
						a0 = -3*(particles.N-1)/2.0*Math.log(Math.PI/particles.springConstant)-Math.log(particles.N)/2.0-Math.log(particles.totalVol);
						control.println("a0 = "+a0);
						a0overN = a0/particles.N;
						a0overNPerVolume = (a0overN*particles.N)/particles.totalVol;
					//	control.println("aOverN = " + a0overN);
						a0overNPerVolumeList.add(a0overNPerVolume);
						/* for computing delta1 */
						uLattice=initialEnergy/particles.N;
	
						ULattices.add(uLattice);
	
						/* update the arrays of the pressures and delta2 */
						meanPressures.add(particles.meanPressure()); // from the varial theorem
						delta2List.add(delta2PerVolume);
					...
					if (lambda==0){
						...
						control.println("boltzmann Factor = "+boltzmannFactorPerN);
						delta1 = uLattice-boltzmannFactorPerN;
						delta1PerVolume = (delta1*particles.N)/particles.totalVol;
						boltzmannFactorPerNList.add(boltzmannFactorPerN);
					//	control.println("delta1 = " + delta1);
					//	control.println("delta2 = " + delta2);
						
						totalFreeEnergy = a0overN+delta1+delta2;
						control.println("totalFreeEnergy = " + totalFreeEnergy);
						totalFreeEnergyPerVolume = (totalFreeEnergy*particles.N)/particles.totalVol;
					//	control.println("totalFreeEnergyPerVolume = " + totalFreeEnergyPerVolume);
						
						/* add up all the free energies together */
						totalSumOfEnergiesPerVolume = totalFreeEnergyPerVolume+floryRehnerfreeEnergyPerVolume;
					//	control.println("totalSumOfEnergies = " + totalSumOfEnergiesPerVolume);
					...
					//	totalVolumes.add(particles.totalVol);
						delta1List.add(delta1PerVolume);
						totalSumOfEnergiesList.add(totalSumOfEnergiesPerVolume);
					//	excessHelmhotzFreeEnergyPerVolumes.add(helmhotzFreeEnergyPerVolume);
	
						dryVolFrac += particles.dphi; //increment the dry volume fraction
						particles.dryVolFrac = dryVolFrac;
						lambda = particles.dlambda*0.5; //reset lambda to be dlambda/2
						particles.lambda = lambda;
						springEnergySumAccumulator = 0;
						this.initialize();
						return;
					}
	
					if (lambda<1){
						springEnergySumAccumulator += particles.springEnergyAccumulator; //accumulate springEnergyAccumulator from 0 to 0.95
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergySolidPhaseForDelta2.txt");
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidChangedVariablesEditedApp());
	}
}
