 ...
 * In this version of the code, I am computing the facet volume for all the interacting microgels
 * 
 */
 ...
 * 
 * @authors Alan Denton 
import java.util.function.Function;and Matt Urich
 * @version 1.2 25-05-2021
 * 
 */

public class HertzSpheresSolidPhaseCopyFloryHertzModelVersion3 {
    ...
    public double boltzmannFactor, squaredDisplacementAccumulator, capToGelVolAccumulator;
    ...
    public boolean facetingMicrogels;
    public Random random = new Random();
    public boolean overlappingStatus[][];
    public boolean overlappingAfterMove[][];
    public boolean overlappingBeforeMove[][];
    public double capVolumes[][];
    public double floryFRJAfterMove[], floryFRJBeforeMove[];
    public double capVolAferMove[], capVolBeforeMove[];
   ...
   public void initialize(String configuration) {
      ...
      capVolBeforeMove = new double[N];
      capVolAferMove = new double[N];
   ...
   */
    ...
    */
    public void step() { // performs a trial move of every particle
      ...
      double floryFRIBeforeMove, floryFRIAfterMove;
      ...
      for (i = 0; i < N; i++) { // one microgel
         ...
         for (j = 0; j < N; j++) {
            ...
            if(j != i) { // consider interactions with other particles
               ...
               if (r2 < sigma*sigma){
                  ...
                  capVolBeforeMove[i] = capVolForI;
                  
                  capVolSumBeforeMove += capVolForI;

                  // Accumulate total overlap volume
            ...
            floryFRJBeforeMove[j] = mixF+elasticF;

         }
         
         // Flory-Rehner single-particle free energy before the move (associated with swelling)
         double newNMon = nMon*(volOfMicrogel[i]-capVolSumBeforeMove)/volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
         ...
         System.out.println("datrial: "+datrial);
         ...
         for (j = 0; j < N; j++) {
            ...
            newNMon = nMon * (volOfMicrogel[j]-capVolForJ)/volOfMicrogel[j]; // number of Monomers to compute the new FR free energy
            mixF = newNMon * ((a[j]*a[j]*a[j]-1)*Math.log(1-1/(a[j]*a[j]*a[j]))+chi*(1-1/(a[j]*a[j]*a[j])));
            elasticF = 1.5 * newNMon * xLinkFrac * (a[j]*a[j]-Math.log(a[j])-1);
         ...
         // the total cap volume associated with microgel i after the move
         newNMon = nMon*(volOfMicrogel[i]-capVolSumAfterMove)/volOfMicrogel[i]; // number of Monomers to compute the new FR free energy
         mixF = newNMon*((a[i]*a[i]*a[i]-1)*Math.log(1-1/(a[i]*a[i]*a[i]))+chi*(1-1/(a[i]*a[i]*a[i])));
         elasticF = 1.5*newNMon*xLinkFrac*(a[i]*a[i]-Math.log(a[i])-1);
         floryFRIAfterMove = mixF+elasticF;
         
         dFlory = floryFRIAfterMove-floryFRIBeforeMove; // change in energy for microgel i
         
         if(j != i){
            // change in energy for microgel j
            for(j = 0; j < N; j++){
               dFlory += floryFRJAfterMove[j]-floryFRJBeforeMove[j];
            }
         }

         de = (pairEnergySum-energy[i])+dFlory;
   ...
   public double meanRadius() { // mean radius [units of dry radius]
    ...
    public double reservoirSwellingRatio(double nMon, double nChains, double chi){
        org.opensourcephysics.numerics.Function f = new FloryRehnerPressure(nMon, nChains, chi);
        double xleft = 1.1;
        double xright = 15.;
    ...
    class FloryRehnerPressure implements org.opensourcephysics.numerics.Function{
