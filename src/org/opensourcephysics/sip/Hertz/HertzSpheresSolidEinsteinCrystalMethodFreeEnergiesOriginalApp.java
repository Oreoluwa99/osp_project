...
public class HertzSpheresSolidEinsteinCrystalMethodFreeEnergiesOriginalApp extends AbstractSimulation {
	public enum WriteModes {
		WRITE_NONE, WRITE_RADIAL, WRITE_ALL;
	};

	HertzSpheresSolidEinsteinCrystalMethod particles = new HertzSpheresSolidEinsteinCrystalMethod(); // with
																										// equilibration
	// HertzSpheresSolidEinsteinCrystalCopy particles = new
	// HertzSpheresSolidEinsteinCrystalCopy(); // without equilibration
	...
	public void initialize() {
		...
		if (structure) {
			rdf = new RDF(particles.x, particles.y, particles.z, particles.side, particles.grBinWidth,
					particles.fileExtension);
			ssf = new SSF(particles.x, particles.y, particles.z, particles.side, particles.d, particles.deltaK,
					particles.fileExtension);
		}
	}
		...
		if (particles.steps == particles.stop) {
			...
			if (dryVolFrac < dryVolFracMax) {
				...
				if (x < upperLimit) {
					...
					if (x > upperLimit) {
						...
						/*
						 * Calculate a0: the addittion of delta3(change between unconstrained solid and
				...
				if (lambda == 0) {
					...
					/*
					 * the free energy change between an interacting einstein crystal and a
					 * non-interacting einstein crystal
					 */
					...
					fPair = f0overN + deltaF1 + deltaF2; // the addition of all the free energy of a molecular solid (the pair energies)
			...
			//if (dryVolFracs.size() > 1) {  // checks whether the size list is greater than 1
				...
				for (i = 1; i < dryVolFracs.size() - 1; i++) {
					...
					calculatedPressures.add(floryPContribution + pairPContribution + 1.0); // the calculated pressure from the derivatives of the free energies
	...
	public void writeData() {
		...
		for (int i = 0; i < particles.maxRadius / particles.grBinWidth; i++) {
			particles.sizeDist[i] = particles.sizeDist[i]
					/ ((particles.stop - particles.delay) / particles.snapshotInterval); // normalize size distribution
		}

		particles.volFrac = particles.volFrac / ((particles.stop - particles.delay) / particles.snapshotInterval); // average
																													// system
																													// volume
																													// fraction																			// in
																													// equilibrium
		...
		try {
			File systemInfo = new File("data/systemInfo2" + particles.fileExtension + ".txt");
		...
		try {
			File sizeFile = new File("data/microgelSize2" + particles.fileExtension + ".txt");
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergySolidEinsteinCrystalMethodFile2.txt");
			...
			bw1.newLine();
			bw1.write(
					"phi0 ,  mu/KT,    (PV/NkT)_total,   1+(PV/NkT)_virial,    1+(PV/NkT)_pair/calculated,     (PV/NkT)_FR,      <F_total>/V,      EntropySolid,	F0/V,       F1/V,     F2/V,     <F_FR>/V,     <alpha>,      zeta, 	deltaF1MinusUlattice");
			bw1.newLine();
	...
	public static void main(String[] args) { // set up animation control
		...
		SimulationControl control = SimulationControl
				.createApp(new HertzSpheresSolidEinsteinCrystalMethodFreeEnergiesOriginalApp());
	}
}
