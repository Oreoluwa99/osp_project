...
public class HertzSpheresSolidPhaseCopyVersion1App extends AbstractSimulation {
	public enum WriteModes {WRITE_NONE, WRITE_RADIAL, WRITE_ALL;};
	// HertzSpheresSolidPhase particles = new HertzSpheresSolidPhase(); 	
	//HertzSpheresSolidPhaseCopy particles = new HertzSpheresSolidPhaseCopy();	//edited by Ore on Sept 03
	...
	public void reset() {
		...
		control.setValue("DryVolFrac Max", 0.017);
		...
		control.setValue("Stop", 60000); // steps after which statistics collection stops
	...
	public void writeData() {
		...
		try {
			...
			bw1.newLine();
			bw1.write("phi0,         Freference/V,           <F_FR>/V,     		<alpha>,      		zeta, 	 		SpringConstant, 		lindemannParameter");
			bw1.newLine();

			for (int i = 1; i < dryVolFracs.size() - 1; i++) {
				double roundedDryVolFrac = Double.parseDouble(decimalFormat.format(dryVolFracs.get(i)));
				bw1.write(roundedDryVolFrac + ", " + referenceFRPerVolList.get(i) + ", " +  floryFEperVolList.get(i) + ", " + swellingRatioList.get(i) + ", " + reservoirVolFracList.get(i) + ", " + springConstantList.get(i) + ", " + lindemannParameterList.get(i));
				bw1.newLine();
			}
			bw1.close();
		} catch (IOException e) {
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresSolidPhaseCopyVersion1App());
	}
}
