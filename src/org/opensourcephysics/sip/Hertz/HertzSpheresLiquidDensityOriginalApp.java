...
public class HertzSpheresLiquidDensityOriginalApp extends AbstractSimulation {
	...
	double residualPart = 0;
	double n;
	double stirlingApproximation, residualPartPerVolume, totalSumOfEnergies;
	...
	List<Double> residualPartPerVolumes = new ArrayList<>();
	...
	*/
	...
	public void doStep() { // logical step in the HertzSpheres class	    	    
		...
		if (particles.steps == particles.stop){
			...
			if (dryVolFrac < dryVolFracMax){
					...
					control.println("stirling Approximation = "+stirlingApproximation);
					stirlingApproximations.add(stirlingApproximation);

					residualPartPerVolume = (residualPart*particles.N)/particles.totalVol;
					control.println("residualPartPerVolume = "+residualPartPerVolume);
					residualPartPerVolumes.add(residualPartPerVolume);

					freeEnergyPerVolume = (particles.meanFreeEnergy() * particles.N)/particles.totalVol;
					control.println("freeEnergyPerVolume = "+freeEnergyPerVolume);
					freeEnergyPerVolumes.add(freeEnergyPerVolume);

					totalSumOfEnergies = freeEnergyPerVolume+freeEnergyIdealPerVolume+residualPartPerVolume+stirlingApproximation;
					control.println("totalSumOfEnergies = "+totalSumOfEnergies);
					totalSums.add(totalSumOfEnergies);
	...
	public void writeData(){
		...
		try {
			File outputFile = new File("data/dryVolFracVsFreeEnergyDensityCopy.txt");
			...
			bw1.newLine();	
			bw1.write("DryVolFrac, SumOfEnergies, residualPartPerVolume, IdealGasFreeEnergy, FloryRehnerFreeEnergy, reservoirVolFrac, meanPressure(), 	newHertzianPotential");
			bw1.newLine();		

			for (int i = 0; i < dryVolFracs.size(); i++) {
				bw1.write(dryVolFracs.get(i) + ", " + totalSums.get(i) + ", " + residualPartPerVolumes.get(i) + ", " + freeEnergyIdealPerVolumes.get(i) + ", " + freeEnergyPerVolumes.get(i) + ", " + reservoirVolFracs.get(i) + ", " + meanPressures.get(i) + ", " + newHertzianPotentialList.get(i));
				bw1.newLine();
			}
	...
	public static void main(String[] args) { // set up animation control
			...
			SimulationControl control = SimulationControl.createApp(new HertzSpheresLiquidDensityOriginalApp());
	}
}
