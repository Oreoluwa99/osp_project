/*
 * The major reason for this code is to account for the non-local 
 * faceting of the microgels in the step() method.
 * 
 */
...
public class HertzSpheresNonLocalFacet_Copy {
   ...
   */
   ...
   public void step() { // performs a trial move of every particle
      ...
      nonOverlappingVol = 0; // initialize the total non overlapping volume
      ...
      for (i = 0; i < N; i++) {
         ...
         double oldVolOfMicrogel = volOfMicrogel[i];  // Store the old volume before the move
         ...
         if (Math.exp(-de) < random.nextDouble()) { // Metropolis algorithm
            ...
            // Use old volume before the move for nonoverlapping volume calculation
            nonOverlappingVol += (volOfMicrogel[i]-capVolSumBeforeMoveArray[i]); // the non overlapping part for each microgel
            ...
            nonOverlappingVol += (volOfMicrogel[i]-capVolSumAfterMoveI); // the non overlapping part for each microgel
            
         }
      }

      // Calculate the volume fraction
      volumeFraction = nonOverlappingVol/totalVol;
      ...
      for(int i = 0; i < N; ++i) {
         ...
         if (steps > delay){
            if ((steps-delay)%snapshotInterval==0){ // include configurations only after even number of intervals
               ...
               volFracAccumulator += volumeFraction;
            }
         }
   }
   ...
   public double reservoirSwellingRatio(double nMon, double nChains, double chi){
      HertzSpheresNonLocalFacet_Copy.FloryRehnerPressure f = new FloryRehnerPressure(nMon, nChains, chi);
      double xleft = 1.1;
      double xright = 15.;
