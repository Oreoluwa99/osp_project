...
public class HertzSpheresSolidPhaseEdited {
    ...
    public double initialEnergy;//stores the initial energy of the particles on their lattice sites
    public double displacement; //distance between the current position and the initial position of each particle
    ...
    public double pairPotentialDifferenceAccumulator;
   ...
   */
   public void initialize(String configuration) {
      ...
      virialAccumulator = 0;
      springEnergyAccumulator = 0; // for the spring energy in the einstein crystal
   ...
   */
   public void setFCCpositions() {
      ...
      calculateTotalEnergy(lambda); // initial energy
      initialEnergy = pairEnergyAccumulator;
   }
  ...
  public void step() { // performs a trial move of every particle
   ...
   for (int i = 0; i < N; i++) { // attempt a trial move (displacement and size change)
      ...
      double trialDisplacementDistance = (dxtrial*dxtrial)+(dytrial*dytrial)+(dztrial*dztrial);
      ...
      double springEnergy0 = springEnergy; //updates the initial spring energy
      ...
      double pairPotentialCorrection = 2*(dxtrial*dxi+dytrial*dyi+dztrial*dzi)+(1-1/(double)N)*trialDisplacementDistance;
      double dSpringEnergy = springEnergy - springEnergy0; //calculates the change in spring energy
         ...
         if(Math.exp(-de) < Math.random()) { // Metropolis algorithm
            ...
            for (int j = 0; j < N; ++j) { // update energies of other particles 
               if(j != i) {
                  ...
                  System.out.println("aceepted move");
               }
            }
   ...
   public void calculateTotalEnergy(double lambda) {
      ...
      for(int i = 0; i < N; ++i) {
         ...
         for(int j = 0; j < N; ++j) {
            if(j != i) { // consider interactions with other particles
               ...
               r2 = xij*xij + yij*yij + zij*zij; // particle separation squared
	            ...
	            if (r2 < sigma*sigma){
                  r = Math.sqrt(r2);
                  // Hertz pair potential amplitude (scaled by a factor of scale*Young) for lambda = 1
                  B = scale*Young*nChains*Math.pow(sigma, 2.)*Math.sqrt(a[i]*a[j])/(Math.pow(a[i], 3.)+Math.pow(a[j], 3.)); 
         ...
         energy[i] = pairEnergySum + totalF + springEnergy; // total energy of particle i
...
*/
      ...
      double boltzmannFactor = Math.exp(initialEnergy-pairEnergyAccumulator);
   ...
   public double einsteinMeanPairEnergy(){//mean pair energy per particle for the einstein crystal
      return (pairEnergyAccumulator/(double)(steps)-springEnergyAccumulator/(double)(steps))/N;
   }
